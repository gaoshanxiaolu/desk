...
/*      All Rights Reserved.                                                  */
/*                                                                            */
/*  RUBYHTP - Ruby Bluetooth Health Thermometer Profile using GATT (LE)       */
/*             application.                                                   */
/*                                                                            */
/*  Author:  Ryan Byrne                                                       */
/*                                                                            */
...
#include "RubyHTP.h"             /* Application Header.                       */
...
#define LE_DEMO_DEVICE_NAME                        "RubyHTP"
...
static void DisplayNotifyIndicateTemperatureUsage(const char *Command)
{
   ...
   PRINTF("[Units (0 = Celsius, 1 = Fahrenheit)] ");
...
static int ConfigureRemoteHTS(ParameterList_t *TempParam)
{
            ...
            if((DeviceInfo = SearchDeviceInfoEntryByBD_ADDR(&DeviceInfoList, ConnectionBD_ADDR)) != NULL)
            {
               ...
               if(DeviceInfo->Flags & DEVICE_INFO_FLAGS_SERVICE_DISCOVERY_COMPLETE)
               {
                  ...
                  if(DeviceInfo->ClientInfo.Temperature_Measurement_Client_Configuration)
                  {
                     ...
                     PRINTF("   Error - Temperature Measurement CC not found on this device.\n");
   ...
   {
      PRINTF("Usage: ConfigureRemoteHTS [Temperature Measurement Indicate (0 = disable, 1 = enable)]\n");
               ...
               if(((DeviceInfo = SearchDeviceInfoEntryByBD_ADDR(&DeviceInfoList, GATT_Client_Event_Data->Event_Data.GATT_Read_Response_Data->RemoteDevice)) != NULL) && (CallbackParameter != 0))
               {
                  ...
                  else
                  {
                     ...
                     else
                     {
                        ...
                        {
                           PRINTF("\nWrite Measurement Interval CC Complete.\n");
                        }
                        else
                        {
                           if(CallbackParameter == DeviceInfo->ClientInfo.Measurement_Interval)
               ...
               if((DeviceInfo = SearchDeviceInfoEntryByBD_ADDR(&DeviceInfoList, GATT_Connection_Event_Data->Event_Data.GATT_Server_Indication_Data->RemoteDevice)) != NULL)
               {
                  ...
                  else
                  {
                     if(DeviceInfo->ClientInfo.Measurement_Interval == GATT_Connection_Event_Data->Event_Data.GATT_Server_Indication_Data->AttributeHandle)
                     {
                        ...
                        PRINTF("Error - Unknown Indication Attribute Handle.\n");
                  }
               }
               else
...
static void RubyHTP_entry(ULONG which_thread)
{
   int                      Result;
...
void user_main(void)
{
   ...
   tx_thread_create(&host_thread, "ruby htp demo thread", RubyHTP_entry, 0, Ptr, PSEUDO_HOST_STACK_SIZE, 16, 16, 4, TX_AUTO_START);
