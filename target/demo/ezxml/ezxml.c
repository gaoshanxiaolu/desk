...
extern int cmnos_vsnprintf(char *str, unsigned int size,const char *fmt, ...);
...
ezxml_t ezxml_err(ezxml_root_t root, char *s, const char *err, ...)
{
    ...
    va_start(ap, err);
    cmnos_vsnprintf(root->err, EZXML_ERRL, fmt, ap); /* Change */
    va_end(ap);
...
char *ezxml_decode(char *s, char **ent, char t)
{
    ...
    if (t == '*') { // normalize spaces for non-cdata attributes
        for (s = r; *s; s++) {
            if ((l = strspn(s, " "))) memmove(s, s + l, strlen(s + l) + 1); /* Change */
            while (*s && *s != ' ') s++;
        }
...
void ezxml_proc_inst(ezxml_root_t root, char *s, size_t len)
{
    ...
    if (! strcmp(target, "xml")) { // <?xml ... ?>
        if ((s = A_STRSTR(s, "standalone")) && ! strncmp(s + strspn(s + 10,
...
short ezxml_internal_dtd(ezxml_root_t root, char *s, size_t len)
{
    ...
    for (s[len] = '\0'; s; ) {
        ...
        else if (! strncmp(s, "<!--", 4)) s = A_STRSTR(s + 4, "-->"); // comments
...
// A wrapper for ezxml_parse_str() that accepts a dset id descriptor. First
// attempts to mem map the file. Failing that, reads the file into memory.
// Returns NULL on failure.
ezxml_t ezxml_parse_fd(A_UINT32 id)
{
    ezxml_root_t root;
    A_UINT32 file_size;
    A_STATUS status;
    ...
    status = dset_open_ex(&dset_handle,id, DSET_MEDIA_NVRAM , NULL, NULL);

    file_size = dset_size(dset_handle);
    m = malloc(file_size);

    if(m == NULL) {
        A_PRINTF_ALWAYS("ERROR: malloc failed \n");
	    return NULL;
    }
     
    status = dset_read(dset_handle, 0, file_size, m);
                      ...
                      size_t *max, short a)
{
    ...
    for (e = s + len; s != e; s++) {
        ...
        switch (*s) {
        case '\0': return *dst;
        case '&': *dlen += qcom_sprintf(*dst + *dlen, "&amp;"); break;
        case '<': *dlen += qcom_sprintf(*dst + *dlen, "&lt;"); break;
        case '>': *dlen += qcom_sprintf(*dst + *dlen, "&gt;"); break;
        case '"': *dlen += qcom_sprintf(*dst + *dlen, (a) ? "&quot;" : "\""); break;
        case '\n': *dlen += qcom_sprintf(*dst + *dlen, (a) ? "&#xA;" : "\n"); break;
        case '\t': *dlen += qcom_sprintf(*dst + *dlen, (a) ? "&#x9;" : "\t"); break;
        case '\r': *dlen += qcom_sprintf(*dst + *dlen, "&#xD;"); break;
        default: (*dst)[(*dlen)++] = *s;
        }
    }
    return *dst;
                    ...
                    size_t start, char ***attr)
{
    ...
    *len += qcom_sprintf(*s + *len, "<%s", xml->name); // open tag
    for (i = 0; xml->attr[i]; i += 2) { // tag attributes
        if (ezxml_attr(xml, xml->attr[i]) != xml->attr[i + 1]) continue;
        while (*len + strlen(xml->attr[i]) + 7 > *max) // reallocate s
            *s = realloc(*s, *max += EZXML_BUFSIZE);

        *len += qcom_sprintf(*s + *len, " %s=\"", xml->attr[i]);
        ezxml_ampencode(xml->attr[i + 1], -1, s, len, max, 1);
        *len += qcom_sprintf(*s + *len, "\"");
    }
    ...
    for (j = 1; attr[i] && attr[i][j]; j += 3) { // default attributes
        ...
        *len += qcom_sprintf(*s + *len, " %s=\"", attr[i][j]);
    ...
    *len += qcom_sprintf(*s + *len, "</%s>", xml->name); // close tag
