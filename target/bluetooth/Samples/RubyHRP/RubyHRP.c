...
/*      All Rights Reserved.                                                  */
/*                                                                            */
/*  RUBYHRP - Ruby Bluetooth Heart Rate Monitor Profile using GATT (LE)       */
/*             application.                                                   */
/*                                                                            */
/*  Author:  Ryan Byrne                                                       */
/*                                                                            */
...
#include "RubyHRP.h"             /* Application Header.                       */
...
#define LE_DEMO_DEVICE_NAME                        "RubyHRP"
...
static int  DecodeDisplayHeartRate(unsigned int ValueLength, Byte_t *Value);
...
static int DecodeDisplayHeartRate(unsigned int ValueLength, Byte_t *Value)
{
   ...
   if((ValueLength) && (Value))
   {
      ...
      Flags                  = ((HRS_Heart_Rate_Measurement_t *)Value)->Header.Flags;
      NumberIntervals        = (Word_t)((ValueLength - HRS_HEART_RATE_MEASUREMENT_LENGTH(Flags, 0)) >> 1);
      MeasurementSize        = HRS_HEART_RATE_MEASUREMENT_DATA_SIZE(NumberIntervals);
      Heart_Rate_Measurement = (HRS_Heart_Rate_Measurement_Data_t *)BTPS_AllocateMemory(MeasurementSize);
      if(Heart_Rate_Measurement)
      {
         Heart_Rate_Measurement->Flags                  = Flags;
         Heart_Rate_Measurement->Heart_Rate             = 0;
         Heart_Rate_Measurement->Energy_Expended        = 0;
         Heart_Rate_Measurement->Number_Of_RR_Intervals = NumberIntervals;

         /* Decode the Heart Rate Measurement.                          */
         ret_val = HRS_Decode_Heart_Rate_Measurement(ValueLength, Value, Heart_Rate_Measurement);
         if(!ret_val)
         {
            PRINTF("      Flags:             0x%02X\n", Flags);
            PRINTF("      Heart Rate:        %u\n", Heart_Rate_Measurement->Heart_Rate);

            if(Flags & HRS_HEART_RATE_MEASUREMENT_FLAGS_ENERGY_EXPENDED_PRESENT)
               PRINTF("      Energy Expended:   %u\n", Heart_Rate_Measurement->Energy_Expended);

            PRINTF("      Sensor Contact:    ");

            if(Flags & HRS_HEART_RATE_MEASUREMENT_FLAGS_SENSOR_CONTACT_STATUS_SUPPORTED)
            {
               if(Flags & HRS_HEART_RATE_MEASUREMENT_FLAGS_SENSOR_CONTACT_STATUS_DETECTED)
                  PRINTF("Detected\n");
               else
                  PRINTF("Not Detected\n");
            }
            ...
            if((Flags & HRS_HEART_RATE_MEASUREMENT_FLAGS_RR_INTERVAL_PRESENT) && (NumberIntervals))
            {
               PRINTF("      Num. RR Intervals: %u\n", NumberIntervals);
               PRINTF("      RR Intervals:      {");
               Index = 0;
               while(Index < NumberIntervals)
               {
                  PRINTF("%s%d", (char *)((Index)?", ":""), Heart_Rate_Measurement->RR_Intervals[Index]);
                  Index++;
               }
               PRINTF("}\n");
            }
            ...
            DisplayFunctionError("HRS_Decode_Heart_Rate_Measurement", ret_val);

         BTPS_FreeMemory(Heart_Rate_Measurement);
   ...
   if(ConnectionID)
   {
      ...
      if((DeviceInfo = SearchDeviceInfoEntryByBD_ADDR(&DeviceInfoList, ConnectionBD_ADDR)) != NULL)
      {
         ...
         if(!(DeviceInfo->Flags & DEVICE_INFO_FLAGS_SERVICE_DISCOVERY_OUTSTANDING))
         {
            ...
            {
               DiscoveryHandleRange.Starting_Handle = (Word_t)TempParam->Params[0].intParam;
               DiscoveryHandleRange.Ending_Handle   = (Word_t)TempParam->Params[1].intParam;
            ...
            if((DeviceInfo = SearchDeviceInfoEntryByBD_ADDR(&DeviceInfoList, ConnectionBD_ADDR)) != NULL)
            {
               ...
               if(DeviceInfo->Flags & DEVICE_INFO_FLAGS_SERVICE_DISCOVERY_COMPLETE)
               {
                  ...
                  if(DeviceInfo->ClientInfo.Heart_Rate_Measurement_Client_Configuration)
                  {
                     ...
                     /* characteristics.                                */
                     ret_val = EnableDisableNotificationsIndications(DeviceInfo->ClientInfo.Heart_Rate_Measurement_Client_Configuration, (Word_t)((TempParam->Params[0].intParam)?GATT_CLIENT_CONFIGURATION_CHARACTERISTIC_NOTIFY_ENABLE:0), GATT_ClientEventCallback_HRP);
                  }
                  else
                     PRINTF("   Error - Heart Rate Measurement CC not found on this device.\n");
                  ...
                  if(HRS_Event_Data->Event_Data.HRS_Read_Client_Configuration_Data->InstanceID == HRSInstanceID)
                  {
                     ...
                     switch(HRS_Event_Data->Event_Data.HRS_Read_Client_Configuration_Data->ClientConfigurationType)
                     {
                        case ctHeartRateMeasurement:
                           PRINTF("   Config Type:      ctHeartReateMeasurement.\n");
            ...
            if(GATT_Client_Event_Data->Event_Data.GATT_Request_Error_Data)
            {
               ...
               if(GATT_Client_Event_Data->Event_Data.GATT_Request_Error_Data->ErrorType == retErrorResponse)
               {
                  ...
                  else
                  {
                     ...
                     {
                        PRINTF("   Error Msg:       Control Point Not Supported.\n");
                     }
                     else
            ...
            if(GATT_Connection_Event_Data->Event_Data.GATT_Server_Notification_Data)
            {
               ...
               if((DeviceInfo = SearchDeviceInfoEntryByBD_ADDR(&DeviceInfoList, GATT_Connection_Event_Data->Event_Data.GATT_Server_Notification_Data->RemoteDevice)) != NULL)
               {
                  if(DeviceInfo->ClientInfo.Heart_Rate_Measurement == GATT_Connection_Event_Data->Event_Data.GATT_Server_Notification_Data->AttributeHandle)
                  {
                     ...
                     Result = DecodeDisplayHeartRate(GATT_Connection_Event_Data->Event_Data.GATT_Server_Notification_Data->AttributeValueLength, GATT_Connection_Event_Data->Event_Data.GATT_Server_Notification_Data->AttributeValue);
                     if(Result)
                        DisplayFunctionError("DecodeDisplayHeartRate", (int)Result);
                  }
                  else
...
static void RubyHRP_entry(ULONG which_thread)
{
   int                      Result;
...
void user_main(void)
{
   ...
   tx_thread_create(&host_thread, "ruby hrp demo thread", RubyHRP_entry, 0, Ptr, PSEUDO_HOST_STACK_SIZE, 16, 16, 4, TX_AUTO_START);
