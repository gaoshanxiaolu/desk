...
#define NUM_11G_CAL_PIERS    12

typedef struct 
{
    A_UINT16 channel;       // 2300 + channel in OTP
    A_INT8   olpcGainDelta;
    A_UINT8  thermCalVal;
} __attribute__((__packed__)) otp_cal;

typedef struct 
{
    int number_of_channel;              // number of valid channels
    otp_cal array[NUM_11G_CAL_PIERS];
}otp_cal_array;
...
A_STATUS _qcom_btcoex_scheme_set(A_UINT8 scheme_id, void* data);
A_INT32 _qcom_firmware_partition_index_get();
A_INT32 _qcom_otp_get_cal_2g(otp_cal_array *ptr);
...
#define qcom_otp_get_cal_2g(ptr) \
    _qcom_otp_get_cal_2g((ptr))
