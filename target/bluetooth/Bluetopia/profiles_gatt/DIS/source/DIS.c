...
#if BTPS_CONFIGURATION_GATT_SUPPORT_SUSPEND_RESUME
   GATT_Attribute_Handle_Group_t InstanceHandleGroup;
#endif
...
#define DIS_SERVER_INSTANCE_DATA_SIZE                    (sizeof(DISServerInstance_t))
   ...
   /* stored/restored in DIS Service Instance when a suspend/resume     */
   /* procedure is performed.                                           */
typedef struct DISSuspendInstanceInfo_s
{
   GATT_Attribute_Handle_Group_t InstanceHandleRange;
   unsigned int                  InstanceID;
   Word_t                        Flags;
   Byte_t                        VariableData[1];
} DISSuspendInstanceInfo_t;

#define DIS_SUSPEND_INSTANCE_INFO_SIZE(_x)               (BTPS_STRUCTURE_OFFSET(DISSuspendInstanceInfo_t, VariableData) + ((_x)*sizeof(Byte_t)))

   /* The following structure contains the information that is          */
   /* stored/restored in DIS Service when a suspend/resume procedure is */
   /* performed.                                                        */
typedef struct DISSuspendInfo_s
{
   unsigned int NumberOfSuspendedInstances;
   Byte_t       VariableData[1];
} DISSuspendInfo_t;

#define DIS_SUSPEND_INFO_SIZE(_x)                        (BTPS_STRUCTURE_OFFSET(DISSuspendInfo_t, VariableData) + ((_x)*sizeof(Byte_t)))

   /* The following flags define the variables which have a non-default */
   /* value stored in the suspend memory for a given DIS instance.      */
#define DIS_SUSPEND_INFO_FLAGS_MANUFACTURER_NAME         0x0001
#define DIS_SUSPEND_INFO_FLAGS_MODEL_NUMBER              0x0002
#define DIS_SUSPEND_INFO_FLAGS_SERIAL_NUMBER             0x0004
#define DIS_SUSPEND_INFO_FLAGS_HARDWARE_REVISION         0x0008
#define DIS_SUSPEND_INFO_FLAGS_FIRMWARE_REVISION         0x0010
#define DIS_SUSPEND_INFO_FLAGS_SOFTWARE_REVISION         0x0020
#define DIS_SUSPEND_INFO_FLAGS_SYSTEM_ID                 0x0040
#define DIS_SUSPEND_INFO_FLAGS_IEEE_CERTIFICATION_DATA   0x0080
#define DIS_SUSPEND_INFO_FLAGS_PNP_ID                    0x0100
...
static int DISRegisterService(unsigned int BluetoothStackID, unsigned int *ServiceID, GATT_Attribute_Handle_Group_t *ServiceHandleRange, unsigned int *PreviousInstanceID);
...
static DISServerInstance_t *AcquireServiceInstance(unsigned int BluetoothStackID, unsigned int *InstanceID)
{
   ...
   /* required for suspend) a given DIS instance.  It returns the number*/
...
static unsigned long SuspendServiceInstance(unsigned int BluetoothStackID, unsigned int InstanceID, unsigned long BufferSize, Byte_t *Buffer)
{
   ...
   DISSuspendInstanceInfo_t *SuspendInstanceInfo;
...
#if BTPS_CONFIGURATION_DIS_SUPPORT_MANUFACTURER_NAME
   RequiredMemory += (InstanceData[InstanceID].Manufacturer_Name.Characteristic_Length + 1);
#endif

#if BTPS_CONFIGURATION_DIS_SUPPORT_MODEL_NUMBER
   RequiredMemory += (InstanceData[InstanceID].Model_Number.Characteristic_Length  + 1);
#endif

#if BTPS_CONFIGURATION_DIS_SUPPORT_SERIAL_NUMBER
   RequiredMemory += (InstanceData[InstanceID].Serial_Number.Characteristic_Length + 1);
#endif

#if BTPS_CONFIGURATION_DIS_SUPPORT_HARDWARE_REVISION
   RequiredMemory += (InstanceData[InstanceID].Hardware_Revision.Characteristic_Length + 1);
#endif

#if BTPS_CONFIGURATION_DIS_SUPPORT_FIRMWARE_REVISION
   RequiredMemory += (InstanceData[InstanceID].Firmware_Revision.Characteristic_Length + 1);
#endif

#if BTPS_CONFIGURATION_DIS_SUPPORT_SOFTWARE_REVISION
   RequiredMemory += (InstanceData[InstanceID].Software_Revision.Characteristic_Length + 1);
#endif

#if BTPS_CONFIGURATION_DIS_SUPPORT_SYSTEM_ID
   RequiredMemory += DIS_SYSTEM_ID_VALUE_DATA_SIZE;
#endif

#if BTPS_CONFIGURATION_DIS_SUPPORT_IEEE_CERTIFICATION_DATA
   RequiredMemory += (InstanceData[InstanceID].IEEE_Certification_Data.Characteristic_Length + 1);
#endif

#if BTPS_CONFIGURATION_DIS_SUPPORT_PNP_ID
   RequiredMemory += DIS_PNP_ID_VALUE_DATA_SIZE;
#endif
   ...
   if(BufferSize)
   {
      ...
      if((BufferSize >= (DIS_SUSPEND_INSTANCE_INFO_SIZE(RequiredMemory) + BTPS_ALIGNMENT_SIZE)) && (Buffer))
      {
         /* Get an aligned pointer to where we will store the suspend   */
         /* information.                                                */
         SuspendInstanceInfo                      = (DISSuspendInstanceInfo_t *)(Buffer + BTPS_ALIGNMENT_OFFSET(Buffer, DISSuspendInstanceInfo_t));

         /* Store the registered GATT Handle Range so we can restore the*/
         /* service in the same location on resume.                     */
         SuspendInstanceInfo->InstanceHandleRange = InstanceList[InstanceID].InstanceHandleGroup;

         /* Save the Instance ID.                                       */
         SuspendInstanceInfo->InstanceID          = InstanceID + 1;

         /* Initialize the flags to 0.                                  */
         SuspendInstanceInfo->Flags               = 0;
         ...
         /* Save the manufacturer's name if it is non-zero.             */
         if((InstanceData[InstanceID].Manufacturer_Name.Characteristic_Length) && (InstanceData[InstanceID].Manufacturer_Name.Data))
         {
            /* Copy the data into the suspend info.                     */
            BTPS_MemCopy(&(SuspendInstanceInfo->VariableData[Offset]), InstanceData[InstanceID].Manufacturer_Name.Data, InstanceData[InstanceID].Manufacturer_Name.Characteristic_Length);

            Offset += InstanceData[InstanceID].Manufacturer_Name.Characteristic_Length;

            /* NULL terminate the string.                               */
            SuspendInstanceInfo->VariableData[Offset] = 0;
            Offset++;

            /* Free the memory allocated for this characteristic.       */
            BTPS_FreeMemory(InstanceData[InstanceID].Manufacturer_Name.Data);

            /* Flag that the manufacturer name is stored.               */
            SuspendInstanceInfo->Flags |= DIS_SUSPEND_INFO_FLAGS_MANUFACTURER_NAME;
         }
         ...
         /* Save the model number if it is non-zero.                    */
         if((InstanceData[InstanceID].Model_Number.Characteristic_Length) && (InstanceData[InstanceID].Model_Number.Data))
         {
            /* Copy the data into the suspend info.                     */
            BTPS_MemCopy(&(SuspendInstanceInfo->VariableData[Offset]), InstanceData[InstanceID].Model_Number.Data, InstanceData[InstanceID].Model_Number.Characteristic_Length);

            Offset += InstanceData[InstanceID].Model_Number.Characteristic_Length;
            ...
            /* Free the memory allocated for this characteristic.       */
            BTPS_FreeMemory(InstanceData[InstanceID].Model_Number.Data);

            /* Flag that the model number is stored.                    */
            SuspendInstanceInfo->Flags |= DIS_SUSPEND_INFO_FLAGS_MODEL_NUMBER;
         }
         ...
         /* Save the serial number if it is non-zero.                   */
         if((InstanceData[InstanceID].Serial_Number.Characteristic_Length) && (InstanceData[InstanceID].Serial_Number.Data))
         {
            /* Copy the data into the suspend info.                     */
            BTPS_MemCopy(&(SuspendInstanceInfo->VariableData[Offset]), InstanceData[InstanceID].Serial_Number.Data, InstanceData[InstanceID].Serial_Number.Characteristic_Length);

            Offset += InstanceData[InstanceID].Serial_Number.Characteristic_Length;
            ...
            /* Free the memory allocated for this characteristic.       */
            BTPS_FreeMemory(InstanceData[InstanceID].Serial_Number.Data);

            /* Flag that the serial number is stored.                   */
            SuspendInstanceInfo->Flags |= DIS_SUSPEND_INFO_FLAGS_SERIAL_NUMBER;
         }
         ...
         /* Save the hardware revision if it is non-zero.               */
         if((InstanceData[InstanceID].Hardware_Revision.Characteristic_Length) && (InstanceData[InstanceID].Hardware_Revision.Data))
         {
            /* Copy the data into the suspend info.                     */
            BTPS_MemCopy(&(SuspendInstanceInfo->VariableData[Offset]), InstanceData[InstanceID].Hardware_Revision.Data, InstanceData[InstanceID].Hardware_Revision.Characteristic_Length);

            Offset += InstanceData[InstanceID].Hardware_Revision.Characteristic_Length;
            ...
            /* Free the memory allocated for this characteristic.       */
            BTPS_FreeMemory(InstanceData[InstanceID].Hardware_Revision.Data);

            /* Flag that the hardware revision is stored.               */
            SuspendInstanceInfo->Flags |= DIS_SUSPEND_INFO_FLAGS_HARDWARE_REVISION;
         }
         ...
         /* Save the Firmware revision if it is non-zero.               */
         if((InstanceData[InstanceID].Firmware_Revision.Characteristic_Length) && (InstanceData[InstanceID].Firmware_Revision.Data))
         {
            /* Copy the data into the suspend info.                     */
            BTPS_MemCopy(&(SuspendInstanceInfo->VariableData[Offset]), InstanceData[InstanceID].Firmware_Revision.Data, InstanceData[InstanceID].Firmware_Revision.Characteristic_Length);

            Offset += InstanceData[InstanceID].Firmware_Revision.Characteristic_Length;
            ...
            /* Free the memory allocated for this characteristic.       */
            BTPS_FreeMemory(InstanceData[InstanceID].Firmware_Revision.Data);

            /* Flag that the Firmware revision is stored.               */
            SuspendInstanceInfo->Flags |= DIS_SUSPEND_INFO_FLAGS_FIRMWARE_REVISION;
         }
         ...
         /* Save the Software revision if it is non-zero.               */
         if((InstanceData[InstanceID].Software_Revision.Characteristic_Length) && (InstanceData[InstanceID].Software_Revision.Data))
         {
            /* Copy the data into the suspend info.                     */
            BTPS_MemCopy(&(SuspendInstanceInfo->VariableData[Offset]), InstanceData[InstanceID].Software_Revision.Data, InstanceData[InstanceID].Software_Revision.Characteristic_Length);

            Offset += InstanceData[InstanceID].Software_Revision.Characteristic_Length;
            ...
            /* Free the memory allocated for this characteristic.       */
            BTPS_FreeMemory(InstanceData[InstanceID].Software_Revision.Data);

            /* Flag that the Software revision is stored.               */
            SuspendInstanceInfo->Flags |= DIS_SUSPEND_INFO_FLAGS_SOFTWARE_REVISION;
         }
         ...
         /* Copy the data into the suspend info.                        */
         BTPS_MemCopy(&(SuspendInstanceInfo->VariableData[Offset]), InstanceData[InstanceID].System_ID.Data, DIS_SYSTEM_ID_VALUE_DATA_SIZE);

         Offset += DIS_SYSTEM_ID_VALUE_DATA_SIZE;

         /* Flag that the Software revision is stored.                  */
         SuspendInstanceInfo->Flags |= DIS_SUSPEND_INFO_FLAGS_SYSTEM_ID;
         ...
         /* Save the software revision if it is non-zero.               */
         if((InstanceData[InstanceID].IEEE_Certification_Data.Characteristic_Length) && (InstanceData[InstanceID].IEEE_Certification_Data.Data))
         {
            /* Copy the data into the suspend info.                     */
            BTPS_MemCopy(&(SuspendInstanceInfo->VariableData[Offset]), InstanceData[InstanceID].IEEE_Certification_Data.Data, InstanceData[InstanceID].IEEE_Certification_Data.Characteristic_Length);

            Offset += InstanceData[InstanceID].IEEE_Certification_Data.Characteristic_Length;
            ...
            /* Free the memory allocated for this characteristic.       */
            BTPS_FreeMemory(InstanceData[InstanceID].IEEE_Certification_Data.Data);

            /* Flag that the software revision is stored.               */
            SuspendInstanceInfo->Flags |= DIS_SUSPEND_INFO_FLAGS_IEEE_CERTIFICATION_DATA;
         }
         ...
         /* Copy the data into the suspend info.                        */
         BTPS_MemCopy(&(SuspendInstanceInfo->VariableData[Offset]), InstanceData[InstanceID].PNP_ID.Data, DIS_PNP_ID_VALUE_DATA_SIZE);

         Offset += DIS_PNP_ID_VALUE_DATA_SIZE;

         /* Flag that the PnP ID is stored.                             */
         SuspendInstanceInfo->Flags |= DIS_SUSPEND_INFO_FLAGS_PNP_ID;

#endif

         /* Mark the instance entry as being free.                   */
         BTPS_MemInitialize(&(InstanceList[InstanceID]), 0, DIS_SERVER_INSTANCE_DATA_SIZE);

         /* Clear the instance data.                                 */
         BTPS_MemInitialize(&(InstanceData[InstanceID]), 0, DIS_INSTANCE_DATA_SIZE);

         /* Calculate the actual required amount of memory used.        */
         RequiredMemory = (DIS_SUSPEND_INSTANCE_INFO_SIZE(Offset) + BTPS_ALIGNMENT_OFFSET(Buffer, DISSuspendInstanceInfo_t));
      }
      else
         RequiredMemory = 0;
   }
   else
   {
      ...
      RequiredMemory += (DIS_SUSPEND_INSTANCE_INFO_SIZE(0) + BTPS_ALIGNMENT_SIZE);
   }
...
static int ResumeServiceInstance(unsigned int BluetoothStackID, unsigned long *BufferSize, Byte_t **Buffer)
{
   ...
   if((TempBufferSize >= DIS_SUSPEND_INSTANCE_INFO_SIZE(0)) && (TempBuffer))
   {
      /* Calculate the alignment offset for the buffer.                 */
      AlignmentOffset = BTPS_ALIGNMENT_OFFSET(TempBuffer, DISSuspendInstanceInfo_t);

      /* Make sure there is enough buffer space to align the buffer.    */
      if(TempBufferSize >= (DIS_SUSPEND_INSTANCE_INFO_SIZE(0) + AlignmentOffset))
      {
         /* Align the temp buffer pointer.                              */
         SuspendInstanceInfo  = (DISSuspendInstanceInfo_t *)(TempBuffer + AlignmentOffset);

         /* Set temp buffer to the variable data.                       */
         TempBuffer           = SuspendInstanceInfo->VariableData;

         /* Subtract the header from the buffer size.                   */
         TempBufferSize      -= (DIS_SUSPEND_INSTANCE_INFO_SIZE(0) + AlignmentOffset);

         /* Verify that the requested InstanceID is not being used      */
         /* currently.                                                  */
         if((SuspendInstanceInfo->InstanceID) && (SuspendInstanceInfo->InstanceID <= DIS_MAXIMUM_SUPPORTED_INSTANCES) && (!(InstanceList[SuspendInstanceInfo->InstanceID-1].ServiceID)))
         {
            /* Attempt to re-initialize the DIS service.                */
            ret_val = DISRegisterService(BluetoothStackID, &ServiceID, &(SuspendInstanceInfo->InstanceHandleRange), &(SuspendInstanceInfo->InstanceID));
            if(ret_val > 0)
            {
               /* Save the initialized Instance ID.                     */
               InstanceID = (unsigned int)ret_val;

               /* Intilize the Instance Data for this new instance.     */
               /* * NOTE * We will make sure that the data is           */
               /*          initialized since some Characteristics may   */
               /*          not be restored.                             */
               ...
               /* See if we need to restore the manufacturer's name.    */
               if(SuspendInstanceInfo->Flags & DIS_SUSPEND_INFO_FLAGS_MANUFACTURER_NAME)
               {
                  ...
                  {
                     /* Attempt to set the manufacturer's name.         */
                     ret_val = DIS_Set_Manufacturer_Name(BluetoothStackID, InstanceID, (char *)TempBuffer);
                     if(!ret_val)
                     {
               ...
               /* See if we need to restore the model number.           */
               if((!ret_val) && (SuspendInstanceInfo->Flags & DIS_SUSPEND_INFO_FLAGS_MODEL_NUMBER))
               {
                  /* Check the length of the stored string.             */
                  StringLength = BTPS_StringLength((BTPSCONST char *)TempBuffer) + 1;
                  ...
                  if(TempBufferSize >= StringLength)
                  {
                     /* Attempt to set the model number.                */
                     ret_val = DIS_Set_Model_Number(BluetoothStackID, InstanceID, (char *)TempBuffer);
                     if(!ret_val)
                     {
                        /* Update the pointers and the buffersize.      */
                        TempBufferSize -= StringLength;
               ...
               /* See if we need to restore the Serial number.          */
               if((!ret_val) && (SuspendInstanceInfo->Flags & DIS_SUSPEND_INFO_FLAGS_SERIAL_NUMBER))
               {
                  /* Check the length of the stored string.             */
                  StringLength = BTPS_StringLength((BTPSCONST char *)TempBuffer) + 1;
                  ...
                  if(TempBufferSize >= StringLength)
                  {
                     ...
                     ret_val = DIS_Set_Serial_Number(BluetoothStackID, InstanceID, (char *)TempBuffer);
                     if(!ret_val)
                     {
                        /* Update the pointers and the buffersize.      */
                        TempBufferSize -= StringLength;
               ...
               /* See if we need to restore the hardware revision.      */
               if((!ret_val) && (SuspendInstanceInfo->Flags & DIS_SUSPEND_INFO_FLAGS_HARDWARE_REVISION))
               {
                  /* Check the length of the stored string.             */
                  StringLength = BTPS_StringLength((BTPSCONST char *)TempBuffer) + 1;
                  ...
                  if(TempBufferSize >= StringLength)
                  {
                     /* Attempt to set the hardware revision.           */
                     ret_val = DIS_Set_Hardware_Revision(BluetoothStackID, InstanceID, (char *)TempBuffer);
                     if(!ret_val)
                     {
                        /* Update the pointers and the buffersize.      */
                        TempBufferSize -= StringLength;
               ...
               /* See if we need to restore the firmware revision.      */
               if((!ret_val) && (SuspendInstanceInfo->Flags & DIS_SUSPEND_INFO_FLAGS_FIRMWARE_REVISION))
               {
                  /* Check the length of the stored string.             */
                  StringLength = BTPS_StringLength((BTPSCONST char *)TempBuffer) + 1;
                  ...
                  if(TempBufferSize >= StringLength)
                  {
                     /* Attempt to set the firmware revision.           */
                     ret_val = DIS_Set_Firmware_Revision(BluetoothStackID, InstanceID, (char *)TempBuffer);
                     if(!ret_val)
                     {
                        /* Update the pointers and the buffersize.      */
                        TempBufferSize -= StringLength;
               ...
               /* See if we need to restore the software revision.      */
               if((!ret_val) && (SuspendInstanceInfo->Flags & DIS_SUSPEND_INFO_FLAGS_SOFTWARE_REVISION))
               {
                  /* Check the length of the stored string.             */
                  StringLength = BTPS_StringLength((BTPSCONST char *)TempBuffer) + 1;
                  ...
                  if(TempBufferSize >= StringLength)
                  {
                     /* Attempt to set the software revision.           */
                     ret_val = DIS_Set_Software_Revision(BluetoothStackID, InstanceID, (char *)TempBuffer);
                     if(!ret_val)
                     {
                        /* Update the pointers and the buffersize.      */
                        TempBufferSize -= StringLength;
               ...
               /* Continue to set the system ID only if no error has    */
               /* already occurred.                                     */
               if((!ret_val) && (SuspendInstanceInfo->Flags & DIS_SUSPEND_INFO_FLAGS_SYSTEM_ID))
               {
                  /* Verify that we have enough memory for the System   */
                  /* ID.                                                */
                  if(TempBufferSize >= DIS_SYSTEM_ID_VALUE_DATA_SIZE)
                  {
                     /* Attempt to set the software revision.           */
                     ret_val = DIS_Set_System_ID(BluetoothStackID, InstanceID, (DIS_System_ID_Value_t *)TempBuffer);
                     if(!ret_val)
                     {
                        /* Update the pointers and the buffersize.      */
                        TempBufferSize -= DIS_SYSTEM_ID_VALUE_DATA_SIZE;
                        TempBuffer     += DIS_SYSTEM_ID_VALUE_DATA_SIZE;
                     }
                  }
                  else
                     ret_val = DIS_ERROR_MALFORMATTED_DATA;
               ...
               /* See if we need to restore the IEEE Certificate Data.  */
               if((!ret_val) && (SuspendInstanceInfo->Flags & DIS_SUSPEND_INFO_FLAGS_IEEE_CERTIFICATION_DATA))
               {
                  /* Check the length of the stored string.             */
                  StringLength = BTPS_StringLength((BTPSCONST char *)TempBuffer) + 1;
                  ...
                  if(TempBufferSize >= StringLength)
                  {
                     /* Attempt to set the IEEE Certificate Data.       */
                     ret_val = DIS_Set_IEEE_Certification_Data(BluetoothStackID, InstanceID, (StringLength - 1), TempBuffer);
                     if(!ret_val)
                     {
                        /* Update the pointers and the buffersize.      */
                        TempBufferSize -= StringLength;
               ...
               /* If no error has occurred and the PnP ID is valid,     */
               /* restore it.                                           */
               if((!ret_val) && (SuspendInstanceInfo->Flags & DIS_SUSPEND_INFO_FLAGS_PNP_ID))
               {
                  /* Verify that we have enough memory for the PnP Data.*/
                  if(TempBufferSize >= DIS_PNP_ID_VALUE_DATA_SIZE)
                  {
                     /* Copy the PnP ID into the instance data (it is   */
                     /* already formatted).                             */
                     BTPS_MemCopy(InstanceData[InstanceID - 1].PNP_ID.Data, TempBuffer, DIS_PNP_ID_VALUE_DATA_SIZE);

                     /* Update the pointers and the buffersize.         */
                     TempBufferSize -= DIS_PNP_ID_VALUE_DATA_SIZE;
                     TempBuffer     += DIS_PNP_ID_VALUE_DATA_SIZE;
                  }
                  else
                     ret_val = DIS_ERROR_MALFORMATTED_DATA;
               }
               ...
               else
               {
                  ...
                  DIS_Cleanup_Service(BluetoothStackID, InstanceID);
               }
            }
         }
         else
...
static int DISRegisterService(unsigned int BluetoothStackID, unsigned int *ServiceID, GATT_Attribute_Handle_Group_t *ServiceHandleRange, unsigned int *PreviousInstanceID)
{
   ...
   unsigned int              Service_Flags = DIS_SERVICE_FLAGS;
   ...
   if((BluetoothStackID) && (ServiceID) && (ServiceHandleRange))
   {
      ...
      if(!InstanceRegisteredByStackID(BluetoothStackID))
      {
         ...
         if((ServiceInstance = AcquireServiceInstance(BluetoothStackID, &InstanceID)) != NULL)
         {
            ...
            /* If BR/EDR is not supported by GATT, then we will only    */
            /* accept the GATT LE Service Flag.                         */
            Service_Flags &= ~(GATT_SERVICE_FLAGS_BR_EDR_SERVICE);
            ...
            /* match the expected (which we know is free) we need    */
            /* update the Instance ID to the old value so we restore */
            /* the suspended service instance information to the     */
            /* expected location.                                    */
            ...
            ret_val = GATT_Register_Service(BluetoothStackID, (Byte_t)Service_Flags, DEVICE_INFORMATION_SERVICE_ATTRIBUTE_COUNT, (GATT_Service_Attribute_Entry_t *)Device_Information_Service, ServiceHandleRange, GATT_ServerEventCallback, InstanceID);
            if(ret_val > 0)
            {
               ...
               /* Store the handle range that the DIS service was saved */
               /* in.                                                   */
               ServiceInstance->InstanceHandleGroup = *ServiceHandleRange;

#endif

               /* Return the GATT Service ID to the caller.             */
               *ServiceID                           = (unsigned int)ret_val;
               ...
               /* Setup the information needed to register the SDP      */
               /* record.                                               */
               SDPServiceRecord.NumberServiceClassUUID = 1;

               SDPUUID.SDP_Data_Element_Type   = deUUID_16;
               DIS_ASSIGN_DIS_SERVICE_SDP_UUID_16(SDPUUID.UUID_Value.UUID_16);
...
int BTPSAPI DIS_Initialize_Service(unsigned int BluetoothStackID, unsigned int *ServiceID)
{
   ...
   return(DISRegisterService(BluetoothStackID, ServiceID, &ServiceHandleRange, NULL));
}
...
unsigned long BTPSAPI DIS_Suspend(unsigned int BluetoothStackID, unsigned long BufferSize, void *Buffer)
{
#if BTPS_CONFIGURATION_GATT_SUPPORT_SUSPEND_RESUME
   ...
   DISSuspendInfo_t *SuspendInfo     = NULL;

   /* Verify that the input parameters are semi-valid.                  */
   if((BufferSize == 0) || ((BufferSize >= (DIS_SUSPEND_INFO_SIZE(0) + BTPS_ALIGNMENT_SIZE)) && (Buffer)))
   {
      ...
      if(!BSC_LockBluetoothStack(BluetoothStackID))
      {
         ...
         if(BufferSize)
         {
            ...
            SuspendInfo                              = (DISSuspendInfo_t *)(TempPtr + BTPS_ALIGNMENT_OFFSET(TempPtr, DISSuspendInfo_t));

            /* Calculated the memory used for the header.               */
            MemoryUsed                               = (DIS_SUSPEND_INFO_SIZE(0) + (((Byte_t *)SuspendInfo) - ((Byte_t *)Buffer)));
         ...
         {
            /* Calculated the memory used for the header.               */
            TotalMemoryUsed +=(DIS_SUSPEND_INFO_SIZE(0));
         ...
         for(Index = 0;Index < DIS_MAXIMUM_SUPPORTED_INSTANCES;Index++)
         {
            ...
            if((InstanceList[Index].BluetoothStackID == BluetoothStackID) && (InstanceList[Index].ServiceID))
            {
               ...
               if(PerformSuspend)
               {
                  ...
                  /* suspend.                                           */
                  if(BufferSize >= DIS_SUSPEND_INSTANCE_INFO_SIZE(0))
                  {
   ...
   /* Bluetooth stack after a succDISful suspend has been performed (see*/
...
int BTPSAPI DIS_Resume(unsigned int BluetoothStackID, unsigned long BufferSize, void *Buffer)
{
#if BTPS_CONFIGURATION_GATT_SUPPORT_SUSPEND_RESUME
   ...
   Byte_t           *TempPtr;
   unsigned int      Index;
   DISSuspendInfo_t *SuspendInfo;

   /* Verify that the input parameters are semi-valid.                  */
   if((BufferSize) && ((TempPtr = (Byte_t *)Buffer) != NULL))
   {
      /* Verify that the buffer size is large enough for DIS.           */
      if(BufferSize >= (DIS_SUSPEND_INFO_SIZE(0) + BTPS_ALIGNMENT_OFFSET(TempPtr, DISSuspendInfo_t)))
      {
         /* Subtract the header and alignment offset from the buffer    */
         /* size.                                                       */
         BufferSize  -= (DIS_SUSPEND_INFO_SIZE(0) + BTPS_ALIGNMENT_OFFSET(TempPtr, DISSuspendInfo_t));

         /* Align the buffer for the DISSuspendInfo_t structrue.        */
         SuspendInfo  = (DISSuspendInfo_t *)(TempPtr + BTPS_ALIGNMENT_OFFSET(TempPtr, DISSuspendInfo_t));
