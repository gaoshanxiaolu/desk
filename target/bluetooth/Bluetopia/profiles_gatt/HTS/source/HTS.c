...
#define HTS_SERVER_INSTANCE_DATA_SIZE                    (sizeof(HTSServerInstance_t))
   ...
   /* stored/restored in HTS Service Instance when a suspend/resume     */
   /* procedure is performed.                                           */
typedef struct HTSSuspendInstanceInfo_s
{
   GATT_Attribute_Handle_Group_t InstanceHandleRange;
...
} HTSSuspendInstanceInfo_t;

#define HTS_SUSPEND_INSTANCE_INFO_SIZE(_x)               (BTPS_STRUCTURE_OFFSET(HTSSuspendInstanceInfo_t, VariableData) + ((_x)*sizeof(Byte_t)))

   /* The following structure contains the information that is          */
   /* stored/restored in HTS Service when a suspend/resume procedure is */
   /* performed.                                                        */
typedef struct HTSSuspendInfo_s
{
   unsigned int NumberOfSuspendedInstances;
   Byte_t       VariableData[1];
} HTSSuspendInfo_t;

#define HTS_SUSPEND_INFO_SIZE(_x)                        (BTPS_STRUCTURE_OFFSET(HTSSuspendInfo_t, VariableData) + ((_x)*sizeof(Byte_t)))
...
static int HTSRegisterService(unsigned int BluetoothStackID, HTS_Event_Callback_t EventCallback, unsigned long CallbackParameter, unsigned int *ServiceID, GATT_Attribute_Handle_Group_t *ServiceHandleRange, unsigned int *PreviousInstanceID);
...
static HTSServerInstance_t *AcquireServiceInstance(unsigned int BluetoothStackID, unsigned int *InstanceID)
{
   ...
   /* required for suspend) a given HTS instance.  It returns the number*/
...
static unsigned long SuspendServiceInstance(unsigned int BluetoothStackID, unsigned int InstanceID, unsigned long BufferSize, Byte_t *Buffer)
{
   ...
   HTSSuspendInstanceInfo_t *SuspendInstanceInfo;
   ...
   /* Include the size of the HTS Temperature Type Instance Data.       */
   RequiredMemory += (NON_ALIGNED_WORD_SIZE + NON_ALIGNED_WORD_SIZE);

#endif

#if BTPS_CONFIGURATION_HTS_SUPPORT_MEASUREMENT_INTERVAL

   /* Include the size of the HTS Measurement Interval Instance Data.   */
   RequiredMemory += (NON_ALIGNED_WORD_SIZE + NON_ALIGNED_WORD_SIZE + NON_ALIGNED_WORD_SIZE + HTS_VALID_RANGE_SIZE);
   ...
   if(BufferSize)
   {
      ...
      if((Buffer) && (BufferSize >= (HTS_SUSPEND_INSTANCE_INFO_SIZE(RequiredMemory) + BTPS_ALIGNMENT_SIZE)))
      {
         /* Get an aligned pointer to where we will store the suspend   */
         /* information.                                                */
         SuspendInstanceInfo                      = (HTSSuspendInstanceInfo_t *)(Buffer + BTPS_ALIGNMENT_OFFSET(Buffer, HTSSuspendInstanceInfo_t));
         ...
         /* Copy the HTS Temperature Type into the suspend info.        */
         /* * NOTE * If this is supported it MUST be the first member of*/
         /*          the Instance Data.                                 */
         BTPS_MemCopy(&(SuspendInstanceInfo->VariableData[Offset]), &(InstanceData[InstanceID]), (NON_ALIGNED_WORD_SIZE + NON_ALIGNED_WORD_SIZE));
         Offset += (NON_ALIGNED_WORD_SIZE + NON_ALIGNED_WORD_SIZE);

#endif

#if BTPS_CONFIGURATION_HTS_SUPPORT_MEASUREMENT_INTERVAL

         /* Copy the HTS Measurement Interval into the suspend info.    */
         /* * NOTE * This may not be the first member of the HTS        */
         /*          Instance Data.                                     */
         BTPS_MemCopy(&(SuspendInstanceInfo->VariableData[Offset]), &(InstanceData[InstanceID].Measurement_Interval_Length), (NON_ALIGNED_WORD_SIZE + NON_ALIGNED_WORD_SIZE + NON_ALIGNED_WORD_SIZE + HTS_VALID_RANGE_SIZE));
         Offset += (NON_ALIGNED_WORD_SIZE + NON_ALIGNED_WORD_SIZE + NON_ALIGNED_WORD_SIZE + HTS_VALID_RANGE_SIZE);

#endif

         /* Calculate the actual required amount of memory used.        */
         RequiredMemory = (HTS_SUSPEND_INSTANCE_INFO_SIZE(Offset) + BTPS_ALIGNMENT_OFFSET(Buffer, HTSSuspendInstanceInfo_t));

         /* Mark the instance entry as being free.                      */
         BTPS_MemInitialize(&(InstanceList[InstanceID]), 0, HTS_SERVER_INSTANCE_DATA_SIZE);
      }
      else
         RequiredMemory = 0;
   }
   else
   {
      ...
      RequiredMemory += (HTS_SUSPEND_INSTANCE_INFO_SIZE(0) + BTPS_ALIGNMENT_SIZE);
   }
...
static int ResumeServiceInstance(unsigned int BluetoothStackID, unsigned long *BufferSize, Byte_t **Buffer)
{
   ...
   if((TempBuffer) && (TempBufferSize >= HTS_SUSPEND_INSTANCE_INFO_SIZE(0)))
   {
      /* Calculate the alignment offset for the buffer.                 */
      AlignmentOffset = BTPS_ALIGNMENT_OFFSET(TempBuffer, HTSSuspendInstanceInfo_t);

      /* Make sure there is enough buffer space to align the buffer.    */
      if(TempBufferSize >= (HTS_SUSPEND_INSTANCE_INFO_SIZE(0) + AlignmentOffset))
      {
         /* Align the temp buffer pointer.                              */
         SuspendInstanceInfo  = (HTSSuspendInstanceInfo_t *)(TempBuffer + AlignmentOffset);

         /* Set temp buffer to the variable data.                       */
         TempBuffer           = SuspendInstanceInfo->VariableData;

         /* Subtract the header from the buffer size.                   */
         TempBufferSize      -= (HTS_SUSPEND_INSTANCE_INFO_SIZE(0) + AlignmentOffset);

         /* Verify that the requested InstanceID is not being used      */
         /* currently.                                                  */
         if((SuspendInstanceInfo->InstanceID) && (SuspendInstanceInfo->InstanceID <= HTS_MAXIMUM_SUPPORTED_INSTANCES) && (!(InstanceList[SuspendInstanceInfo->InstanceID-1].ServiceID)))
         {
            /* Attempt to re-initialize the HTS service.                */
            ret_val = HTSRegisterService(BluetoothStackID, SuspendInstanceInfo->EventCallback, SuspendInstanceInfo->CallbackParameter, &ServiceID, &(SuspendInstanceInfo->InstanceHandleRange), &(SuspendInstanceInfo->InstanceID));
            if(ret_val > 0)
            {
               ...
               /* Verify that we have enough memory for the HTS Instance*/
               /* Data.                                                 */
               if(TempBufferSize >= (NON_ALIGNED_WORD_SIZE + NON_ALIGNED_WORD_SIZE))
               {
                  /* Copy the suspended instance data into the HTS      */
                  /* Instance Data (it is already formatted).           */
                  BTPS_MemCopy(&(InstanceData[InstanceID - 1]), TempBuffer, (NON_ALIGNED_WORD_SIZE + NON_ALIGNED_WORD_SIZE));

                  /* Update the pointers and the buffersize.            */
                  TempBufferSize -= (NON_ALIGNED_WORD_SIZE + NON_ALIGNED_WORD_SIZE);
                  TempBuffer     += (NON_ALIGNED_WORD_SIZE + NON_ALIGNED_WORD_SIZE);
               }
               else
                  ret_val = HTS_ERROR_MALFORMATTED_DATA;
               ...
               /* Verify that we have enough memory for the HTS Instance*/
               /* Data.                                                 */
               if(TempBufferSize >= (NON_ALIGNED_WORD_SIZE + NON_ALIGNED_WORD_SIZE + NON_ALIGNED_WORD_SIZE + HTS_VALID_RANGE_SIZE))
               {
                  /* Copy the suspended instance data into the HTS      */
                  /* Instance Data (it is already formatted).           */
                  BTPS_MemCopy(&(InstanceData[InstanceID - 1].Measurement_Interval_Length), TempBuffer, (NON_ALIGNED_WORD_SIZE + NON_ALIGNED_WORD_SIZE + NON_ALIGNED_WORD_SIZE + HTS_VALID_RANGE_SIZE));

                  /* Update the pointers and the buffersize.            */
                  TempBufferSize -= (NON_ALIGNED_WORD_SIZE + NON_ALIGNED_WORD_SIZE + NON_ALIGNED_WORD_SIZE + HTS_VALID_RANGE_SIZE);
                  TempBuffer     += (NON_ALIGNED_WORD_SIZE + NON_ALIGNED_WORD_SIZE + NON_ALIGNED_WORD_SIZE + HTS_VALID_RANGE_SIZE);
               }
               else
                  ret_val = HTS_ERROR_MALFORMATTED_DATA;
               ...
               else
               {
                  ...
                  HTS_Cleanup_Service(BluetoothStackID, InstanceID);
               }
            }
         }
         else
...
int BTPSAPI HTS_Initialize_Service(unsigned int BluetoothStackID, HTS_Event_Callback_t EventCallback, unsigned long CallbackParameter, unsigned int *ServiceID)
{
   ...
   return(HTSRegisterService(BluetoothStackID, EventCallback, CallbackParameter, ServiceID, &ServiceHandleRange, NULL));
}
...
unsigned long BTPSAPI HTS_Suspend(unsigned int BluetoothStackID, unsigned long BufferSize, void *Buffer)
{
#if BTPS_CONFIGURATION_GATT_SUPPORT_SUSPEND_RESUME
   ...
   HTSSuspendInfo_t *SuspendInfo     = NULL;

   /* Verify that the input parameters are semi-valid.                  */
   if((BufferSize == 0) || ((Buffer) && (BufferSize >= (HTS_SUSPEND_INFO_SIZE(0) + BTPS_ALIGNMENT_SIZE))))
   {
      ...
      if(!BSC_LockBluetoothStack(BluetoothStackID))
      {
         ...
         if(BufferSize)
         {
            ...
            SuspendInfo                              = (HTSSuspendInfo_t *)(TempPtr + BTPS_ALIGNMENT_OFFSET(TempPtr, HTSSuspendInfo_t));

            /* Calculated the memory used for the header.               */
            MemoryUsed                               = (HTS_SUSPEND_INFO_SIZE(0) + (((Byte_t *)SuspendInfo) - ((Byte_t *)Buffer)));
         ...
         {
            /* Calculated the memory used for the header.               */
            TotalMemoryUsed += (HTS_SUSPEND_INFO_SIZE(0));
         ...
         for(Index = 0; Index < (unsigned int)HTS_MAXIMUM_SUPPORTED_INSTANCES; Index++)
         {
            ...
            if((InstanceList[Index].BluetoothStackID == BluetoothStackID) && (InstanceList[Index].ServiceID))
            {
               ...
               if(PerformSuspend)
               {
                  ...
                  /* suspend.                                           */
                  if(BufferSize >= HTS_SUSPEND_INSTANCE_INFO_SIZE(0))
                  {
...
int BTPSAPI HTS_Resume(unsigned int BluetoothStackID, unsigned long BufferSize, void *Buffer)
{
#if BTPS_CONFIGURATION_GATT_SUPPORT_SUSPEND_RESUME
   ...
   Byte_t           *TempPtr;
   unsigned int      Index;
   HTSSuspendInfo_t *SuspendInfo;

   /* Verify that the input parameters are semi-valid.                  */
   if((BufferSize) && ((TempPtr = (Byte_t *)Buffer) != NULL))
   {
      /* Verify that the buffer size is large enough for HTS.           */
      if(BufferSize >= (HTS_SUSPEND_INFO_SIZE(0) + BTPS_ALIGNMENT_OFFSET(TempPtr, HTSSuspendInfo_t)))
      {
         /* Subtract the header and alignment offset from the buffer    */
         /* size.                                                       */
         BufferSize  -= (HTS_SUSPEND_INFO_SIZE(0) + BTPS_ALIGNMENT_OFFSET(TempPtr, HTSSuspendInfo_t));

         /* Align the buffer for the HTSSuspendInfo_t structrue.        */
         SuspendInfo  = (HTSSuspendInfo_t *)(TempPtr + BTPS_ALIGNMENT_OFFSET(TempPtr, HTSSuspendInfo_t));
   ...
   if((BluetoothStackID) && (InstanceID) && (ConnectionID) && (Temperature_Measurement))
   {
      ...
      if((ServiceInstance = AcquireServiceInstance(BluetoothStackID, &InstanceID)) != NULL)
      {
         ...
         if((NotificationData = BTPS_AllocateMemory(NotificationDataLength)) != NULL)
         {
            ...
            if((ret_val = FormatTemperatureMeasurement(Temperature_Measurement, NotificationDataLength, NotificationData)) == 0)
            {
   ...
   if((BluetoothStackID) && (InstanceID) && (ConnectionID) && (Temperature_Measurement))
   {
      ...
      if((ServiceInstance = AcquireServiceInstance(BluetoothStackID, &InstanceID)) != NULL)
      {
         ...
         if(!(ServiceInstance->TemperatureMeasurementIndicactionInfo.ConnectionID))
         {
            ...
            {
               /* Format the notification.                              */
               if((ret_val = FormatTemperatureMeasurement(Temperature_Measurement, IndicationDataLength, IndicationData)) == 0)
               {
                  /* Attempt to send the notification.                  */
