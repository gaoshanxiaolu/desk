...
/* Customer dedicated API with HF timer.
...
/* There should only be one HF timer during system run. And it should not be used with >1second timeout
** considering power consume. HF time latency is <5us when timeout>100us. 
** fn :timeout handler
** arg: args of fn
** timeout: microsecond, set by user
** type: ONESHOT . PERIODIC should be careful.
** ret: 0 =OK, 1 = error
*/
A_UINT32 qcom_hf_timer_start(void (*fn) (void *), void *arg, A_UINT32 timeout, A_UINT32 type);
void qcom_hf_timer_stop();
