...
#define TPS_SERVER_INSTANCE_DATA_SIZE                    (sizeof(TPSServerInstance_t))
   ...
   /* stored/restored in TPS Service Instance when a suspend/resume     */
   /* procedure is performed.                                           */
typedef struct TPSSuspendInstanceInfo_s
{
   GATT_Attribute_Handle_Group_t InstanceHandleRange;
   unsigned int                  InstanceID;
   Byte_t                        VariableData[1];
} TPSSuspendInstanceInfo_t;

#define TPS_SUSPEND_INSTANCE_INFO_SIZE(_x)               (BTPS_STRUCTURE_OFFSET(TPSSuspendInstanceInfo_t, VariableData) + ((_x)*sizeof(Byte_t)))

   /* The following structure contains the information that is          */
   /* stored/restored in TPS Service when a suspend/resume procedure is */
   /* performed.                                                        */
typedef struct TPSSuspendInfo_s
{
   unsigned int NumberOfSuspendedInstances;
   Byte_t       VariableData[1];
} TPSSuspendInfo_t;

#define TPS_SUSPEND_INFO_SIZE(_x)                        (BTPS_STRUCTURE_OFFSET(TPSSuspendInfo_t, VariableData) + ((_x)*sizeof(Byte_t)))
...
static int TPSRegisterService(unsigned int BluetoothStackID, unsigned int *ServiceID, GATT_Attribute_Handle_Group_t *ServiceHandleRange, unsigned int *PreviousInstanceID);
...
static TPSServerInstance_t *AcquireServiceInstance(unsigned int BluetoothStackID, unsigned int *InstanceID)
{
   ...
   /* required for suspend) a given TPS instance.  It returns the number*/
...
static unsigned long SuspendServiceInstance(unsigned int BluetoothStackID, unsigned int InstanceID, unsigned long BufferSize, Byte_t *Buffer)
{
   ...
   TPSSuspendInstanceInfo_t *SuspendInstanceInfo;
   ...
   /* Include the size of the TPS Instance Data.                        */
   RequiredMemory += TPS_INSTANCE_DATA_SIZE;

   /* Check to see if we need to save the required instance.            */
   if(BufferSize)
   {
      /* Verify that the buffer is big enough.                          */
      if((Buffer) && (BufferSize >= (TPS_SUSPEND_INSTANCE_INFO_SIZE(RequiredMemory) + BTPS_ALIGNMENT_SIZE)))
      {
         /* Get an aligned pointer to where we will store the suspend   */
         /* information.                                                */
         SuspendInstanceInfo                      = (TPSSuspendInstanceInfo_t *)(Buffer + BTPS_ALIGNMENT_OFFSET(Buffer, TPSSuspendInstanceInfo_t));
         ...
         BTPS_MemCopy(&(SuspendInstanceInfo->VariableData[Offset]), &(InstanceData[InstanceID]), TPS_INSTANCE_DATA_SIZE);
         Offset += TPS_INSTANCE_DATA_SIZE;

         /* Calculate the actual required amount of memory used.        */
         RequiredMemory = (TPS_SUSPEND_INSTANCE_INFO_SIZE(Offset) + BTPS_ALIGNMENT_OFFSET(Buffer, TPSSuspendInstanceInfo_t));

         /* Mark the instance entry as being free.                      */
         BTPS_MemInitialize(&(InstanceList[InstanceID]), 0, TPS_SERVER_INSTANCE_DATA_SIZE);
      }
      else
         RequiredMemory = 0;
   }
   else
   {
      ...
      RequiredMemory += (TPS_SUSPEND_INSTANCE_INFO_SIZE(0) + BTPS_ALIGNMENT_SIZE);
   }
...
static int ResumeServiceInstance(unsigned int BluetoothStackID, unsigned long *BufferSize, Byte_t **Buffer)
{
   ...
   if((TempBuffer) && (TempBufferSize >= TPS_SUSPEND_INSTANCE_INFO_SIZE(0)))
   {
      /* Calculate the alignment offset for the buffer.                 */
      AlignmentOffset = BTPS_ALIGNMENT_OFFSET(TempBuffer, TPSSuspendInstanceInfo_t);

      /* Make sure there is enough buffer space to align the buffer.    */
      if(TempBufferSize >= (TPS_SUSPEND_INSTANCE_INFO_SIZE(0) + AlignmentOffset))
      {
         /* Align the temp buffer pointer.                              */
         SuspendInstanceInfo  = (TPSSuspendInstanceInfo_t *)(TempBuffer + AlignmentOffset);

         /* Set temp buffer to the variable data.                       */
         TempBuffer           = SuspendInstanceInfo->VariableData;

         /* Subtract the header from the buffer size.                   */
         TempBufferSize      -= (TPS_SUSPEND_INSTANCE_INFO_SIZE(0) + AlignmentOffset);

         /* Verify that the requested InstanceID is not being used      */
         /* currently.                                                  */
         if((SuspendInstanceInfo->InstanceID) && (SuspendInstanceInfo->InstanceID <= TPS_MAXIMUM_SUPPORTED_INSTANCES) && (!(InstanceList[SuspendInstanceInfo->InstanceID-1].ServiceID)))
         {
            /* Attempt to re-initialize the TPS service.                */
            ret_val = TPSRegisterService(BluetoothStackID, &ServiceID, &(SuspendInstanceInfo->InstanceHandleRange), &(SuspendInstanceInfo->InstanceID));
            if(ret_val > 0)
            {
               ...
               /* Verify that we have enough memory for the TPS Instance*/
               /* Data.                                                 */
               if(TempBufferSize >= TPS_INSTANCE_DATA_SIZE)
               {
                  /* Copy the suspended instance data into the TPS      */
                  /* Instance Data (it is already formatted).           */
                  BTPS_MemCopy(&(InstanceData[InstanceID - 1]), TempBuffer, TPS_INSTANCE_DATA_SIZE);

                  /* Update the pointers and the buffersize.            */
                  TempBufferSize -= TPS_INSTANCE_DATA_SIZE;
                  TempBuffer     += TPS_INSTANCE_DATA_SIZE;
               }
               else
                  ret_val = TPS_ERROR_MALFORMATTED_DATA;
               ...
               else
               {
                  ...
                  TPS_Cleanup_Service(BluetoothStackID, InstanceID);
               }
            }
         }
         else
...
int BTPSAPI TPS_Initialize_Service(unsigned int BluetoothStackID, unsigned int *ServiceID)
{
   ...
   return(TPSRegisterService(BluetoothStackID, ServiceID, &ServiceHandleRange, NULL));
}
...
unsigned long BTPSAPI TPS_Suspend(unsigned int BluetoothStackID, unsigned long BufferSize, void *Buffer)
{
#if BTPS_CONFIGURATION_GATT_SUPPORT_SUSPEND_RESUME
   ...
   TPSSuspendInfo_t *SuspendInfo     = NULL;

   /* Verify that the input parameters are semi-valid.                  */
   if((BufferSize == 0) || ((Buffer) && (BufferSize >= (TPS_SUSPEND_INFO_SIZE(0) + BTPS_ALIGNMENT_SIZE))))
   {
      ...
      if(!BSC_LockBluetoothStack(BluetoothStackID))
      {
         ...
         if(BufferSize)
         {
            ...
            SuspendInfo                              = (TPSSuspendInfo_t *)(TempPtr + BTPS_ALIGNMENT_OFFSET(TempPtr, TPSSuspendInfo_t));

            /* Calculated the memory used for the header.               */
            MemoryUsed                               = (TPS_SUSPEND_INFO_SIZE(0) + (((Byte_t *)SuspendInfo) - ((Byte_t *)Buffer)));
         ...
         {
            /* Calculated the memory used for the header.               */
            TotalMemoryUsed += (TPS_SUSPEND_INFO_SIZE(0));
         ...
         for(Index = 0; Index < (unsigned int)TPS_MAXIMUM_SUPPORTED_INSTANCES; Index++)
         {
            ...
            if((InstanceList[Index].BluetoothStackID == BluetoothStackID) && (InstanceList[Index].ServiceID))
            {
               ...
               if(PerformSuspend)
               {
                  ...
                  /* suspend.                                           */
                  if(BufferSize >= TPS_SUSPEND_INSTANCE_INFO_SIZE(0))
                  {
...
int BTPSAPI TPS_Resume(unsigned int BluetoothStackID, unsigned long BufferSize, void *Buffer)
{
#if BTPS_CONFIGURATION_GATT_SUPPORT_SUSPEND_RESUME
   ...
   Byte_t           *TempPtr;
   unsigned int      Index;
   TPSSuspendInfo_t *SuspendInfo;

   /* Verify that the input parameters are semi-valid.                  */
   if((BufferSize) && ((TempPtr = (Byte_t *)Buffer) != NULL))
   {
      /* Verify that the buffer size is large enough for TPS.           */
      if(BufferSize >= (TPS_SUSPEND_INFO_SIZE(0) + BTPS_ALIGNMENT_OFFSET(TempPtr, TPSSuspendInfo_t)))
      {
         /* Subtract the header and alignment offset from the buffer    */
         /* size.                                                       */
         BufferSize  -= (TPS_SUSPEND_INFO_SIZE(0) + BTPS_ALIGNMENT_OFFSET(TempPtr, TPSSuspendInfo_t));

         /* Align the buffer for the TPSSuspendInfo_t structrue.        */
         SuspendInfo  = (TPSSuspendInfo_t *)(TempPtr + BTPS_ALIGNMENT_OFFSET(TempPtr, TPSSuspendInfo_t));
