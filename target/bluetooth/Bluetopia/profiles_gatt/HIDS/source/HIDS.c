...
#define HID_SERVICE_INFO_DATA_SIZE                       (sizeof(HIDS_Service_Info_t))
   ...
   /* stored/restored in HIDS Service Instance when a suspend/resume     */
   /* procedure is performed.                                           */
typedef struct HIDSSuspendInstanceInfo_s
{
   GATT_Attribute_Handle_Group_t InstanceHandleRange;
   unsigned int                  InstanceID;
   HIDS_Event_Callback_t         EventCallback;
   ...
   unsigned int                  NumExternalReferences;
   unsigned int                  NumReports;
   unsigned long                 IncludedServiceListOffset;
   unsigned long                 GATTUUIDListOffset;
   unsigned long                 ReportReferenceListOffset;
   Byte_t                        VariableData[1];
} HIDSSuspendInstanceInfo_t;

#define HIDS_SUSPEND_INSTANCE_INFO_SIZE(_x)              (BTPS_STRUCTURE_OFFSET(HIDSSuspendInstanceInfo_t, VariableData) + ((_x)*sizeof(Byte_t)))

   /* The following structure contains the information that is          */
   /* stored/restored in HIDS Service when a suspend/resume procedure is */
   /* performed.                                                        */
typedef struct HIDSSuspendInfo_s
{
   unsigned int NumberOfSuspendedInstances;
   Byte_t       VariableData[1];
} HIDSSuspendInfo_t;

#define HIDS_SUSPEND_INFO_SIZE(_x)                       (BTPS_STRUCTURE_OFFSET(HIDSSuspendInfo_t, VariableData) + ((_x)*sizeof(Byte_t)))
...
static HIDS_Service_Info_t *AcquireServiceInstance(unsigned int BluetoothStackID, unsigned int InstanceID)
{
   ...
   /* required for suspend) a given HIDS instance.  It returns the      */
...
static unsigned long SuspendServiceInstance(unsigned int BluetoothStackID, HIDS_Service_Info_t *ServiceInfo, unsigned long BufferSize, Byte_t *Buffer)
{
   ...
   HIDSSuspendInstanceInfo_t *SuspendInstanceInfo;
   unsigned int               Index;

   /* Calculate the memory required for a successful suspend.           */

   /* Include the size for the Included Services List.                  */
   RequiredMemory += (ServiceInfo->NumberIncludedServices * sizeof(unsigned int));

   /* Include the size for the External References List.                */
   RequiredMemory += (ServiceInfo->NumExternalReferences * GATT_UUID_DATA_SIZE);

   /* Include the size of Report Reference List.                        */
   RequiredMemory += (ServiceInfo->NumReports * HIDS_REPORT_REFERENCE_DATA_SIZE);

   /* Check to see if we need to save the required instance.            */
   if(BufferSize)
   {
      /* Verify that the buffer is big enough.                          */
      if((Buffer) && (BufferSize >= (HIDS_SUSPEND_INSTANCE_INFO_SIZE(RequiredMemory) + BTPS_ALIGNMENT_SIZE)))
      {
         /* Get an aligned pointer to where we will store the suspend   */
         /* information.                                                */
         SuspendInstanceInfo                      = (HIDSSuspendInstanceInfo_t *)(Buffer + BTPS_ALIGNMENT_OFFSET(Buffer, HIDSSuspendInstanceInfo_t));

         /* Store the registered GATT Handle Range so we can restore the*/
         /* service in the same location on resume.                     */
         SuspendInstanceInfo->InstanceHandleRange = ServiceInfo->InstanceHandleRange;

         /* Save the Instance ID.                                       */
         /* * NOTE * We need to increment the instance ID since we      */
         /*          previously decremented it.                         */
         SuspendInstanceInfo->InstanceID                     = ServiceInfo->InstanceID;

         /* Save the user Event Callback.                               */
         SuspendInstanceInfo->EventCallback                  = ServiceInfo->EventCallback;

         /* Save the user Callback Parameter.                           */
         SuspendInstanceInfo->CallbackParameter              = ServiceInfo->CallbackParameter;

         /* Save the user Flags.                                        */
         SuspendInstanceInfo->Flags                          = ServiceInfo->Flags;

         /* Save the HID Information Data.                              */
         SuspendInstanceInfo->HIDInformation.Version         = READ_UNALIGNED_WORD_LITTLE_ENDIAN(&(ServiceInfo->HIDServiceInformation.HID_Version));
         SuspendInstanceInfo->HIDInformation.CountryCode     = READ_UNALIGNED_BYTE_LITTLE_ENDIAN(&(ServiceInfo->HIDServiceInformation.CountryCode));
         SuspendInstanceInfo->HIDInformation.Flags           = READ_UNALIGNED_BYTE_LITTLE_ENDIAN(&(ServiceInfo->HIDServiceInformation.Flags));

         /* Save the Number Of Includes Services.                       */
         SuspendInstanceInfo->NumberIncludedServices         = ServiceInfo->NumberIncludedServices;

         /* Save the Number Of External Report References.              */
         SuspendInstanceInfo->NumExternalReferences          = ServiceInfo->NumExternalReferences;

         /* Save the Number Of Reports.                                 */
         SuspendInstanceInfo->NumReports                     = ServiceInfo->NumReports;

         /* Set the save offsets to zero.                               */
         SuspendInstanceInfo->IncludedServiceListOffset      = 0;
         SuspendInstanceInfo->GATTUUIDListOffset             = 0;
         SuspendInstanceInfo->ReportReferenceListOffset      = 0;

         /* Set the initialize offset to 0.                             */
         Offset                                              = 0;

         /* Calculate the actual required amount of memory used.        */
         RequiredMemory = (HIDS_SUSPEND_INSTANCE_INFO_SIZE(Offset) + BTPS_ALIGNMENT_OFFSET(Buffer, HIDSSuspendInstanceInfo_t));

         /* Store the Included Service List in the suspend info.        */
         /* * NOTE * The Offset will always be zero.                    */
         for(Index = 0; Index < ServiceInfo->NumberIncludedServices; Index++)
         {
            /* Store the Service ID for the included service.           */
            *((unsigned int *)&(SuspendInstanceInfo->VariableData[Offset])) = ServiceInfo->IncludedServiceList[Index].ServiceID;
            Offset += sizeof(unsigned int);
         }

         /* Store the External Report References List in the suspend    */
         /* info.                                                       */
         SuspendInstanceInfo->GATTUUIDListOffset = Offset;
         for(Index = 0; Index < ServiceInfo->NumExternalReferences; Index++)
         {
            /* Store the GATT UUID for the External Report Reference.   */
            *((GATT_UUID_t *)&(SuspendInstanceInfo->VariableData[Offset])) = ServiceInfo->ExternalReportReferenceList[Index].UUID;
            Offset += GATT_UUID_DATA_SIZE;

         }

         /* Store the Report Reference List in the suspend info.        */
         SuspendInstanceInfo->ReportReferenceListOffset = Offset;
         for(Index = 0; Index < ServiceInfo->NumExternalReferences; Index++)
         {
            /* Store the Report Reference data.                         */
            *((HIDS_Report_Reference_Data_t *)&(SuspendInstanceInfo->VariableData[Offset])) = *((HIDS_Report_Reference_Data_t *)&(ServiceInfo->ReportReferenceInfoList[Index].ReportReferenceInfo));
            Offset += HIDS_REPORT_REFERENCE_DATA_SIZE;
         }

         /* Delete the HID Service Info entry and free the allocated    */
         /* memory.                                                     */
         /* * NOTE * The caller already has acquired the list lock.     */
         if((ServiceInfo = DeleteHIDServiceInfoEntry(&HIDServiceInfoList, ServiceInfo->InstanceID)) != NULL)
         {
            /* Free the service information for this instance.          */
            FreeHIDServiceInfoEntryMemory(ServiceInfo);
         }
         else
            RequiredMemory = 0;
   ...
   else
   {
      ...
      RequiredMemory += (HIDS_SUSPEND_INSTANCE_INFO_SIZE(0) + BTPS_ALIGNMENT_SIZE);
   }
...
static int ResumeServiceInstance(unsigned int BluetoothStackID, unsigned long *BufferSize, Byte_t **Buffer)
{
   ...
   HIDSSuspendInstanceInfo_t *SuspendInstanceInfo;
   HIDS_Service_Info_t       *ServiceInfo = NULL;

   /* Make sure we have enough memory for an instance header.           */
   if((TempBuffer) && (TempBufferSize >= HIDS_SUSPEND_INSTANCE_INFO_SIZE(0)))
   {
      /* Calculate the alignment offset for the buffer.                 */
      AlignmentOffset = BTPS_ALIGNMENT_OFFSET(TempBuffer, HIDSSuspendInstanceInfo_t);

      /* Make sure there is enough buffer space to align the buffer.    */
      if(TempBufferSize >= (HIDS_SUSPEND_INSTANCE_INFO_SIZE(0) + AlignmentOffset))
      {
         /* Align the temp buffer pointer.                              */
         SuspendInstanceInfo  = (HIDSSuspendInstanceInfo_t *)(TempBuffer + AlignmentOffset);

         /* Set temp buffer to the variable data.                       */
         TempBuffer           = SuspendInstanceInfo->VariableData;

         /* Subtract the header from the buffer size.                   */
         TempBufferSize      -= (HIDS_SUSPEND_INSTANCE_INFO_SIZE(0) + AlignmentOffset);

         /* We need to go ahead re-initialize the HIDS service.         */
         ret_val = HIDSRegisterService(BluetoothStackID,
                                       SuspendInstanceInfo->Flags,
                                       &(SuspendInstanceInfo->HIDInformation),
                                       SuspendInstanceInfo->NumberIncludedServices,
                                       (unsigned int *)(TempBuffer + SuspendInstanceInfo->IncludedServiceListOffset),
                                       SuspendInstanceInfo->NumExternalReferences,
                                       (GATT_UUID_t *)(TempBuffer + SuspendInstanceInfo->GATTUUIDListOffset),
                                       SuspendInstanceInfo->NumReports,
                                       (HIDS_Report_Reference_Data_t *)(TempBuffer + SuspendInstanceInfo->ReportReferenceListOffset),
                                       SuspendInstanceInfo->EventCallback,
                                       SuspendInstanceInfo->CallbackParameter,
                                       &ServiceID,
                                       &(SuspendInstanceInfo->InstanceHandleRange),
                                       &(SuspendInstanceInfo->InstanceID));
         if((ret_val > 0) && (ServiceInfo))
         {
            ...
            /* We need to account for the size of the service table.    */
            TempBufferSize -= (ServiceInfo->NumberServiceAttributeEntries * GATT_SERVICE_ATTRIBUTE_ENTRY_DATA_SIZE);
            TempBuffer     += (ServiceInfo->NumberServiceAttributeEntries * GATT_SERVICE_ATTRIBUTE_ENTRY_DATA_SIZE);

            /* Verify that we have enough memory for the Number of      */
            /* Included Services.                                       */
            if(TempBufferSize >= (ServiceInfo->NumberIncludedServices * GATT_INCLUDE_DEFINITION_ENTRY_DATA_SIZE))
            {
               /* Copy the Number of Included Services into the HIDS    */
               /* Instance Data (it is already formatted).              */
               BTPS_MemCopy(&(ServiceInfo->IncludedServiceList), TempBuffer, (ServiceInfo->NumberIncludedServices * GATT_INCLUDE_DEFINITION_ENTRY_DATA_SIZE));

               /* Update the pointers and the buffersize.               */
               TempBufferSize -= (ServiceInfo->NumberIncludedServices * GATT_INCLUDE_DEFINITION_ENTRY_DATA_SIZE);
               TempBuffer     += (ServiceInfo->NumberIncludedServices * GATT_INCLUDE_DEFINITION_ENTRY_DATA_SIZE);
            }
            else
            ...
            if(!ret_val)
            {
               /* Verify that we have enough memory for the Number of   */
               /* Included Services.                                    */
               if(TempBufferSize >= (ServiceInfo->NumExternalReferences * HIDS_HID_EXTERNAL_REPORT_REFERENCE_DATA_SIZE))
               {
                  /* Copy the External Report Reference List into the   */
                  /* HIDS Instance Data (it is already formatted).      */
                  BTPS_MemCopy(&(ServiceInfo->ExternalReportReferenceList), TempBuffer, (ServiceInfo->NumExternalReferences * HIDS_HID_EXTERNAL_REPORT_REFERENCE_DATA_SIZE));

                  /* Update the pointers and the buffersize.            */
                  TempBufferSize -= (ServiceInfo->NumExternalReferences * HIDS_HID_EXTERNAL_REPORT_REFERENCE_DATA_SIZE);
                  TempBuffer     += (ServiceInfo->NumExternalReferences * HIDS_HID_EXTERNAL_REPORT_REFERENCE_DATA_SIZE);
               }
               else
                  ret_val = HIDS_ERROR_MALFORMATTED_DATA;
            ...
            if(!ret_val)
            {
               /* Verify that we have enough memory for the Number of   */
               /* Included Services.                                    */
               if(TempBufferSize >= (ServiceInfo->NumReports * HIDS_HID_REPORT_REFERENCE_DATA_SIZE))
               {
                  /* Copy the Report Reference List into the HIDS       */
                  /* Instance Data (it is already formatted).           */
                  BTPS_MemCopy(&(ServiceInfo->ReportReferenceInfoList), TempBuffer, (ServiceInfo->NumReports * HIDS_HID_REPORT_REFERENCE_DATA_SIZE));

                  /* Update the pointers and the buffersize.            */
                  TempBufferSize -= (ServiceInfo->NumReports * HIDS_HID_REPORT_REFERENCE_DATA_SIZE);
                  TempBuffer     += (ServiceInfo->NumReports * HIDS_HID_REPORT_REFERENCE_DATA_SIZE);
               }
               else
                  ret_val = HIDS_ERROR_MALFORMATTED_DATA;
            ...
            /* Update the caller's buffer size and point for any further*/
            /* instances.                                               */
...
static int HIDSRegisterService(unsigned int BluetoothStackID,
                               Byte_t Flags,
                               HIDS_HID_Information_Data_t *HIDInformation,
                               unsigned int NumIncludedServices,
                               unsigned int *ServiceIDList,
                               unsigned int NumExternalReportReferences,
                               GATT_UUID_t *ReferenceUUID,
                               unsigned int NumReports,
                               ...
                               unsigned int *PreviousInstanceID)
{
   int                  ret_val;
   Byte_t               ReportType;
   ...
   if((BluetoothStackID) && (HIDInformation) && ((NumIncludedServices == 0) || ((NumIncludedServices) && (ServiceIDList))) && ((NumExternalReportReferences == 0) || ((NumExternalReportReferences) && (ReferenceUUID))) && ((NumReports == 0) || ((NumReports) && (ReportReference))) && (EventCallback) && (ServiceID))
   {
      ...
      if(!BSC_LockBluetoothStack(BluetoothStackID))
      {
         ...
         {
            if((ReportReference[i].ReportType == HIDS_REPORT_REFERENCE_REPORT_TYPE_INPUT_REPORT) || (ReportReference[i].ReportType == (HIDS_REPORT_REFERENCE_REPORT_TYPE_INPUT_REPORT | HIDS_REPORT_REFERENCE_SUPPORT_INPUT_WRITE_FEATURE)))
         ...
         if(!ret_val)
         {
            ...
            if(BSC_AcquireListLock())
            {
               ...
               if(ServiceInfo)
               {
                  ...
                  /* If the Instance ID of the initialized service      */
                  /* doesn't match the expected (which we know is free) */
                  /* we need update the Instance ID to the old value so */
                  /* we restore the suspended service instance          */
                  /* information to the expected location.              */
                  if((PreviousInstanceID) && (*PreviousInstanceID != ServiceInfo->InstanceID))
                  {
                     /* Save the InstanceID we should use from now on.  */
                     ServiceInfo->InstanceID = *PreviousInstanceID;
                  }
                  ...
                  if((ret_val = ConfigureHIDServiceInfo(ServiceInfo, NumIncludedServices, NumExternalReportReferences, NumReports, NumberAttributeEntries)) == 0)
                  {
                     ...
                     /* Characteristic.                                 */
                     for(i=0;i<NumExternalReportReferences;i++)
                     {
                     ...
                     if(!ret_val)
                     {
                        ...
                        if(ret_val > 0)
                        {
                           ...
                           *ServiceID                       = (unsigned int)ret_val;
                           ...
                           /* Store the handle range that the service   */
                           /* was saved in.                             */
                           ServiceInfo->InstanceHandleRange = *ServiceHandleRange;

                           /* Store the Flags used to register the      */
                           /* service.                                  */
...
int BTPSAPI HIDS_Initialize_Service(unsigned int BluetoothStackID, Byte_t Flags, HIDS_HID_Information_Data_t *HIDInformation, unsigned int NumIncludedServices, unsigned int *ServiceIDList, unsigned int NumExternalReportReferences, GATT_UUID_t *ReferenceUUID, unsigned int NumReports, HIDS_Report_Reference_Data_t *ReportReference, HIDS_Event_Callback_t EventCallback, unsigned long CallbackParameter, unsigned int *ServiceID)
{
   ...
   return(HIDSRegisterService(BluetoothStackID, Flags, HIDInformation, NumIncludedServices, ServiceIDList, NumExternalReportReferences, ReferenceUUID, NumReports, ReportReference, EventCallback, CallbackParameter, ServiceID, &ServiceHandleRange, NULL));
}
...
unsigned long BTPSAPI HIDS_Suspend(unsigned int BluetoothStackID, unsigned long BufferSize, void *Buffer)
{
#if BTPS_CONFIGURATION_GATT_SUPPORT_SUSPEND_RESUME
   ...
   HIDSSuspendInfo_t   *SuspendInfo     = NULL;
   HIDS_Service_Info_t *ServiceInfo;
   HIDS_Service_Info_t *NextServiceInfo;

   /* Verify that the input parameters are semi-valid.                  */
   if((BufferSize == 0) || ((Buffer) && (BufferSize >= (HIDS_SUSPEND_INFO_SIZE(0) + BTPS_ALIGNMENT_SIZE))))
   {
      ...
      if(!BSC_LockBluetoothStack(BluetoothStackID))
      {
         ...
         if(BufferSize)
         {
            ...
            SuspendInfo                              = (HIDSSuspendInfo_t *)(TempPtr + BTPS_ALIGNMENT_OFFSET(TempPtr, HIDSSuspendInfo_t));

            /* Calculated the memory used for the header.               */
            MemoryUsed                               = (HIDS_SUSPEND_INFO_SIZE(0) + (((Byte_t *)SuspendInfo) - ((Byte_t *)Buffer)));
         ...
         {
            /* Calculated the memory used for the header.               */
            TotalMemoryUsed += (HIDS_SUSPEND_INFO_SIZE(0));
         ...
         if(BSC_AcquireListLock())
         {
            ...
            ServiceInfo = HIDServiceInfoList;
            while((ServiceInfo) && (MemoryUsed != 0))
            {
               /* Get the next service instance entry.                  */
               /* * NOTE * We need to do this so that we don't reference*/
               /*          the ServiceInfo if it is going to be freed.  */
               NextServiceInfo = ServiceInfo->NextHIDServiceInfoPtr;
               ...
               if(PerformSuspend)
               {
                  ...
                  /* suspend.                                           */
                  if(BufferSize >= HIDS_SUSPEND_INSTANCE_INFO_SIZE(0))
                  {
                     /* Suspend this service.                           */
                     /* ** NOTE ** ServiceInfo MUST not be referenced   */
                     /*            after calling this function.  The    */
                     /*            memory will be freed if this function*/
                     /*            is successful.                       */
                     MemoryUsed = SuspendServiceInstance(BluetoothStackID, ServiceInfo, BufferSize, TempPtr);
               ...
               else
               {
                  ...
                  /* successful suspend.                                */
                  TotalMemoryUsed += SuspendServiceInstance(BluetoothStackID, ServiceInfo, 0, NULL);
               }

               /* Set the next service instance entry.                  */
...
int BTPSAPI HIDS_Resume(unsigned int BluetoothStackID, unsigned long BufferSize, void *Buffer)
{
#if BTPS_CONFIGURATION_GATT_SUPPORT_SUSPEND_RESUME
   ...
   Byte_t            *TempPtr;
   unsigned int       Index;
   HIDSSuspendInfo_t *SuspendInfo;

   /* Verify that the input parameters are semi-valid.                  */
   if((BufferSize) && ((TempPtr = (Byte_t *)Buffer) != NULL))
   {
      /* Verify that the buffer size is large enough for HIDS.          */
      if(BufferSize >= (HIDS_SUSPEND_INFO_SIZE(0) + BTPS_ALIGNMENT_OFFSET(TempPtr, HIDSSuspendInfo_t)))
      {
         /* Subtract the header and alignment offset from the buffer    */
         /* size.                                                       */
         BufferSize  -= (HIDS_SUSPEND_INFO_SIZE(0) + BTPS_ALIGNMENT_OFFSET(TempPtr, HIDSSuspendInfo_t));

         /* Align the buffer for the HIDSSuspendInfo_t structrue.       */
         SuspendInfo  = (HIDSSuspendInfo_t *)(TempPtr + BTPS_ALIGNMENT_OFFSET(TempPtr, HIDSSuspendInfo_t));
