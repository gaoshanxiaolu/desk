...
#define APPLICATION_FLAG_ADVERTISING_ENABLED           0x10
...
static void WriteEIRInformation(char *LocalName)
{
   ...
   /* The following is responsible for opening an SPP Server on the     */
   /* default SPP Port.                                                 */
static int RegisterSPP(void)
{
   int   ret_val;
         ...
         case etAuthentication:
            ...
            switch(GAP_Event_Data->Event_Data.GAP_Authentication_Event_Data->GAP_Authentication_Event_Type)
            {
               case atLinkKeyRequest:
                  ...
                  {
                     PRINTF("GAP_Authentication_Response %d\n", Result);
                  }
                  break;
   ...
   if((SPP_Event_Data) && (BluetoothStackID))
   {
      ...
      switch(SPP_Event_Data->Event_Data_Type)
      {
         ...
         case etPort_Transmit_Buffer_Empty_Indication:
            PRINTF("\etPort_Transmit_Buffer_Empty_Indication.\n");
            break;
         default:
...
int bluetooth_server_handler(int argc, char *argv[])
{
   ...
   if(argc == 1)
   {
      PRINTF("bluetooth_server_handler {<Mask>}\n");
      PRINTF("   where 0x00 - Disable Bluetooth\n");
...
#define BYTE_POOL_SIZE                       (6*1024)
#define PSEUDO_HOST_STACK_SIZE               (5*1024)
...
static void Bluetooth_entry(ULONG which_thread)
{
   char           *argv[] ={"App", "3", 0};
   ...
   if(console_fd >= 0)
   {
      ...
      if(Result >= 0)
      {
         LAPU = (Word_t)((BluetoothAddress >> 16) & 0x00FF);
         LAPL = (Word_t)(BluetoothAddress & 0xFFFF);
      ...
      bluetooth_server_handler(3, argv);
...
void user_main(void)
{
   ...
   tx_thread_create(&host_thread, "ruby bluetooh demo thread", Bluetooth_entry, 0, Ptr, PSEUDO_HOST_STACK_SIZE, 16, 16, 4, TX_AUTO_START);
