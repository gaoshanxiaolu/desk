...
/*      All Rights Reserved.                                                  */
/*                                                                            */
/*  RUBYHOGP - Ruby Bluetooth HID Over GATT Profile using GATT (LE)           */
/*              application.                                                  */
/*                                                                            */
/*  Author:  Tim Cook                                                         */
/*                                                                            */
...
/******************************************************************************/
#include "RubyHOGP.h"            /* Application Header.                       */
...
#define DEFAULT_DEBUG_LOG_FILE_NAME                 "RubyHOGP_ASC.log"
#define DEFAULT_DEBUG_FTS_FILE_NAME                 "RubyHOGP_FTS.log"

   /* Determine the Name we will use for this application.              */
#define DEFAULT_LOCAL_DEVICE_NAME                   "RubyHOGP"
...
#define DEVICE_INFO_FLAGS_IRK_VALID                         0x04
...
static int SendPairingRequest(BD_ADDR_t BD_ADDR, Boolean_t ConnectionMaster)
{
   ...
   if(BluetoothStackID)
   {
      ...
      if(!COMPARE_NULL_BD_ADDR(BD_ADDR))
      {
         ...
         /* Set the BD_ADDR of the device that we are attempting to     */
         /* pair with.                                                  */
...
static int RegisterHIDS(ParameterList_t *TempParam)
{
   ...
   /* The following function is responsible for sending a key button.   */
...
static int GetBatteryLevel(ParameterList_t *TempParam)
{
   ...
   /* The following function sets the current battery level.            */
static int SetBatteryLevel(ParameterList_t *TempParam)
{
   int ret_val;
   Byte_t        Level;
               ...
               switch(Authentication_Event_Data->GAP_LE_Authentication_Event_Type)
               {
                  case latLongTermKeyRequest:
                     ...
                     else
                     {
                        ...
                        /* Since we failed to generate the requested    */
                        /* key we should respond with a negative        */
                        /* response.                                    */
                        GAP_LE_Authentication_Response_Information.GAP_LE_Authentication_Type = larLongTermKey;
                  ...
                  if(DeviceInfo->Flags & DEVICE_INFO_FLAGS_LINK_ENCRYPTED)
                  {
                     ...
                     else
                     {
                        ...
                        else
                        {
                           ...
                           {
                              /* Respond with the Keyboard Output       */
                              /* Report.  Note that since our Report    */
                              /* Mode Report is identical to the Boot   */
                              /* Mode Report we do not need to          */
                              /* differentiate here.                    */
...
static void RubyHOGP_entry(ULONG which_thread)
{
   int                      Result;
...
void user_main(void)
{
   ...
   tx_thread_create(&host_thread, "ruby hogp demo thread", RubyHOGP_entry, 0, Ptr, PSEUDO_HOST_STACK_SIZE, 16, 16, 4, TX_AUTO_START);
