...
#define HRS_SERVER_INSTANCE_DATA_SIZE                    (sizeof(HRSServerInstance_t))
   ...
   /* stored/restored in HRS Service Instance when a suspend/resume     */
   /* procedure is performed.                                           */
typedef struct HRSSuspendInstanceInfo_s
{
   GATT_Attribute_Handle_Group_t InstanceHandleRange;
   unsigned int                  InstanceID;
   HRS_Event_Callback_t          EventCallback;
   unsigned long                 CallbackParameter;
   unsigned long                 SupportedCommands;
   Byte_t                        VariableData[1];
} HRSSuspendInstanceInfo_t;

#define HRS_SUSPEND_INSTANCE_INFO_SIZE(_x)               (BTPS_STRUCTURE_OFFSET(HRSSuspendInstanceInfo_t, VariableData) + ((_x)*sizeof(Byte_t)))

   /* The following structure contains the information that is          */
   /* stored/restored in HRS Service when a suspend/resume procedure is */
   /* performed.                                                        */
typedef struct HRSSuspendInfo_s
{
   unsigned int NumberOfSuspendedInstances;
   Byte_t       VariableData[1];
} HRSSuspendInfo_t;

#define HRS_SUSPEND_INFO_SIZE(_x)                        (BTPS_STRUCTURE_OFFSET(HRSSuspendInfo_t, VariableData) + ((_x)*sizeof(Byte_t)))
...
static int HRSRegisterService(unsigned int BluetoothStackID, unsigned long Supported_Commands, HRS_Event_Callback_t EventCallback, unsigned long CallbackParameter, unsigned int *ServiceID, GATT_Attribute_Handle_Group_t *ServiceHandleRange, unsigned int *PreviousInstanceID);
...
static HRSServerInstance_t *AcquireServiceInstance(unsigned int BluetoothStackID, unsigned int *InstanceID)
{
   ...
   /* required for suspend) a given HRS instance.  It returns the number*/
...
static unsigned long SuspendServiceInstance(unsigned int BluetoothStackID, unsigned int InstanceID, unsigned long BufferSize, Byte_t *Buffer)
{
   ...
   HRSSuspendInstanceInfo_t *SuspendInstanceInfo;
   ...
   /* Include the size of the HRS Instance Data.                        */
   RequiredMemory += HRS_INSTANCE_DATA_SIZE;
   ...
   if(BufferSize)
   {
      ...
      if((Buffer) && (BufferSize >= (HRS_SUSPEND_INSTANCE_INFO_SIZE(RequiredMemory) + BTPS_ALIGNMENT_SIZE)))
      {
         /* Get an aligned pointer to where we will store the suspend   */
         /* information.                                                */
         SuspendInstanceInfo                      = (HRSSuspendInstanceInfo_t *)(Buffer + BTPS_ALIGNMENT_OFFSET(Buffer, HRSSuspendInstanceInfo_t));
         ...
         /* Save the user Supported Commands.                           */
         SuspendInstanceInfo->SupportedCommands   = InstanceList[InstanceID].Supported_Commands;
         ...
         BTPS_MemCopy(&(SuspendInstanceInfo->VariableData[Offset]), &(InstanceData[InstanceID]), HRS_INSTANCE_DATA_SIZE);
         Offset += HRS_INSTANCE_DATA_SIZE;

#endif

         /* Calculate the actual required amount of memory used.        */
         RequiredMemory = (HRS_SUSPEND_INSTANCE_INFO_SIZE(Offset) + BTPS_ALIGNMENT_OFFSET(Buffer, HRSSuspendInstanceInfo_t));

         /* Mark the instance entry as being free.                      */
         BTPS_MemInitialize(&(InstanceList[InstanceID]), 0, HRS_SERVER_INSTANCE_DATA_SIZE);
      }
      else
         RequiredMemory = 0;
   }
   else
   {
      ...
      RequiredMemory += (HRS_SUSPEND_INSTANCE_INFO_SIZE(0) + BTPS_ALIGNMENT_SIZE);
   }
...
static int ResumeServiceInstance(unsigned int BluetoothStackID, unsigned long *BufferSize, Byte_t **Buffer)
{
   ...
   if((TempBuffer) && (TempBufferSize >= HRS_SUSPEND_INSTANCE_INFO_SIZE(0)))
   {
      /* Calculate the alignment offset for the buffer.                 */
      AlignmentOffset = BTPS_ALIGNMENT_OFFSET(TempBuffer, HRSSuspendInstanceInfo_t);

      /* Make sure there is enough buffer space to align the buffer.    */
      if(TempBufferSize >= (HRS_SUSPEND_INSTANCE_INFO_SIZE(0) + AlignmentOffset))
      {
         /* Align the temp buffer pointer.                              */
         SuspendInstanceInfo  = (HRSSuspendInstanceInfo_t *)(TempBuffer + AlignmentOffset);

         /* Set temp buffer to the variable data.                       */
         TempBuffer           = SuspendInstanceInfo->VariableData;

         /* Subtract the header from the buffer size.                   */
         TempBufferSize      -= (HRS_SUSPEND_INSTANCE_INFO_SIZE(0) + AlignmentOffset);

         /* Verify that the requested InstanceID is not being used      */
         /* currently.                                                  */
         if((SuspendInstanceInfo->InstanceID) && (SuspendInstanceInfo->InstanceID <= HRS_MAXIMUM_SUPPORTED_INSTANCES) && (!(InstanceList[SuspendInstanceInfo->InstanceID-1].ServiceID)))
         {
            /* Attempt to re-initialize the HRS service.                */
            ret_val = HRSRegisterService(BluetoothStackID, SuspendInstanceInfo->SupportedCommands, SuspendInstanceInfo->EventCallback, SuspendInstanceInfo->CallbackParameter, &ServiceID, &(SuspendInstanceInfo->InstanceHandleRange), &(SuspendInstanceInfo->InstanceID));
            if(ret_val > 0)
            {
               ...
               /* Verify that we have enough memory for the HRS Instance*/
               /* Data.                                                 */
               if(TempBufferSize >= HRS_INSTANCE_DATA_SIZE)
               {
                  /* Copy the suspended instance data into the HRS      */
                  /* Instance Data (it is already formatted).           */
                  BTPS_MemCopy(&(InstanceData[InstanceID - 1]), TempBuffer, HRS_INSTANCE_DATA_SIZE);

                  /* Update the pointers and the buffersize.            */
                  TempBufferSize -= HRS_INSTANCE_DATA_SIZE;
                  TempBuffer     += HRS_INSTANCE_DATA_SIZE;
               }
               else
                  ret_val = HRS_ERROR_MALFORMATTED_DATA;
               ...
               else
               {
                  ...
                  HRS_Cleanup_Service(BluetoothStackID, InstanceID);
               }
            }
         }
         else
                  ...
                  if(!(GATT_ServerEventData->Event_Data.GATT_Read_Request_Data->AttributeValueOffset))
                  {
                     if(Heart_Rate_Service[AttributeOffset].Attribute_Entry_Type == aetCharacteristicDescriptor16)
                     {
                        ...
                        if(EventData)
                        {
                           ...
                           EventData->Event_Data.HRS_Read_Client_Configuration_Data->ClientConfigurationType = ctHeartRateMeasurement;
                  ...
                  if((!(GATT_ServerEventData->Event_Data.GATT_Write_Request_Data->AttributeValueOffset)) && (!(GATT_ServerEventData->Event_Data.GATT_Write_Request_Data->DelayWrite)))
                  {
                     if(Heart_Rate_Service[AttributeOffset].Attribute_Entry_Type == aetCharacteristicDescriptor16)
                     {
                        ...
                        if(EventData)
                        {
                           ...
                           EventData->Event_Data.HRS_Client_Configuration_Update_Data->ClientConfigurationType = ctHeartRateMeasurement;
...
int BTPSAPI HRS_Initialize_Service(unsigned int BluetoothStackID, unsigned long Supported_Commands, HRS_Event_Callback_t EventCallback, unsigned long CallbackParameter, unsigned int *ServiceID)
{
   ...
   return(HRSRegisterService(BluetoothStackID, Supported_Commands, EventCallback, CallbackParameter, ServiceID, &ServiceHandleRange, NULL));
}
...
unsigned long BTPSAPI HRS_Suspend(unsigned int BluetoothStackID, unsigned long BufferSize, void *Buffer)
{
#if BTPS_CONFIGURATION_GATT_SUPPORT_SUSPEND_RESUME
   ...
   HRSSuspendInfo_t *SuspendInfo     = NULL;

   /* Verify that the input parameters are semi-valid.                  */
   if((BufferSize == 0) || ((Buffer) && (BufferSize >= (HRS_SUSPEND_INFO_SIZE(0) + BTPS_ALIGNMENT_SIZE))))
   {
      ...
      if(!BSC_LockBluetoothStack(BluetoothStackID))
      {
         ...
         if(BufferSize)
         {
            ...
            SuspendInfo                              = (HRSSuspendInfo_t *)(TempPtr + BTPS_ALIGNMENT_OFFSET(TempPtr, HRSSuspendInfo_t));

            /* Calculated the memory used for the header.               */
            MemoryUsed                               = (HRS_SUSPEND_INFO_SIZE(0) + (((Byte_t *)SuspendInfo) - ((Byte_t *)Buffer)));
         ...
         {
            /* Calculated the memory used for the header.               */
            TotalMemoryUsed += (HRS_SUSPEND_INFO_SIZE(0));
         ...
         for(Index = 0; Index < (unsigned int)HRS_MAXIMUM_SUPPORTED_INSTANCES; Index++)
         {
            ...
            if((InstanceList[Index].BluetoothStackID == BluetoothStackID) && (InstanceList[Index].ServiceID))
            {
               ...
               if(PerformSuspend)
               {
                  ...
                  /* suspend.                                           */
                  if(BufferSize >= HRS_SUSPEND_INSTANCE_INFO_SIZE(0))
                  {
...
int BTPSAPI HRS_Resume(unsigned int BluetoothStackID, unsigned long BufferSize, void *Buffer)
{
#if BTPS_CONFIGURATION_GATT_SUPPORT_SUSPEND_RESUME
   ...
   Byte_t           *TempPtr;
   unsigned int      Index;
   HRSSuspendInfo_t *SuspendInfo;

   /* Verify that the input parameters are semi-valid.                  */
   if((BufferSize) && ((TempPtr = (Byte_t *)Buffer) != NULL))
   {
      /* Verify that the buffer size is large enough for HRS.           */
      if(BufferSize >= (HRS_SUSPEND_INFO_SIZE(0) + BTPS_ALIGNMENT_OFFSET(TempPtr, HRSSuspendInfo_t)))
      {
         /* Subtract the header and alignment offset from the buffer    */
         /* size.                                                       */
         BufferSize  -= (HRS_SUSPEND_INFO_SIZE(0) + BTPS_ALIGNMENT_OFFSET(TempPtr, HRSSuspendInfo_t));

         /* Align the buffer for the HRSSuspendInfo_t structrue.        */
         SuspendInfo  = (HRSSuspendInfo_t *)(TempPtr + BTPS_ALIGNMENT_OFFSET(TempPtr, HRSSuspendInfo_t));
      ...
      if((ServiceInstance = AcquireServiceInstance(BluetoothStackID, &InstanceID)) != NULL)
      {
         ...
         if((ret_val = GATT_Query_Connection_MTU(BluetoothStackID, ConnectionID, &MTU)) == 0)
         {
            ...
            if((NotificationData = BTPS_AllocateMemory(NotificationDataLength)) != NULL)
            {
               ...
               /* Notification.                                         */
               if((ret_val = FormatHeartRateMeasurement(Heart_Rate_Measurement, NotificationDataLength, NotificationData)) == 0)
               {
