...
########################################
# Generate xxx.out MD5
########################################

md5_out_file="dset_image_md5.bin"

gen_md5_out_file()
{
	echo "gen_md5_out_file"
	echo $QONS_DIR
	cd $QONS_DIR
	...
	rm -f $md5_out_file
  	...
  	#Create MD5sum image file
	add_word 0x$(echo $MD5 | cut -b 1-8) $md5_out_file
	add_word 0x$(echo $MD5 | cut -b 9-16) $md5_out_file
	add_word 0x$(echo $MD5 | cut -b 17-24) $md5_out_file
	add_word 0x$(echo $MD5 | cut -b 25-32) $md5_out_file
	echo "gen_md5_out_file end"
}
...
if !(grep -Fxq "dset_image_md5.bin" "$QONS_DIR"/dsets.txt); then
	echo '0xa05 $QONS_DIR/dset_image_md5.bin' >> "$QONS_DIR"/dsets.txt
fi

########################################
# Generate xxx.out MD5 need to know APPS_OUTFILE 
########################################
